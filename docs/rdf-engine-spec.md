...
1. Query template cache：内存模板记录、metrics、stats、idle TTL 和 bytes 估算已完成，不改变查询语义；materialized result 入口已复用同一模板 key 并写入 cache row / explain；后续把 compiled SQL 入口复用同一模板 key。
