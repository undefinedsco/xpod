...
详细 extension 边界、hot operators 清单、custom index access method 设计和 benchmark
gate 见 [PG RDF Extension Spec](pg-rdf-extension-spec.md)。本节保留总体路线摘要和与
`SolidRdfEngine` 的关系口径。
当前可引用的性能结论和重置/迁移 runbook 见
...
- 当前 PG-backed query result cache 第一版已落在 `PostgresRdfEngine` 内：缓存表为
  ...
  标记实际 cache operator，缺失或失败时保持 baseline table path。`pg-hot-operators`
  ...
  `scan.exact_graph` / `scan.graph_prefix` / `scan.term_in` 且查询无排序、分页、DISTINCT、
  同 pattern 变量相等约束时，`PostgresRdfEngine` 会调用 `xpod_rdf.scan_quads(...)` /
  `xpod_rdf.count_quads(...)`。BGP join、count / numeric aggregate 仍是后续下沉到 C/Rust
  extension 的优化阶段。
...
| Operator | 输入 | 输出 | 用途 |
| --- | --- | --- | --- |
| `xpod_rdf.scan` | graph/source scope、term-id pattern、filter、limit/order hint | term-id row stream + metrics | single-pattern scan / candidate scan |
| `xpod_rdf.bgp_join` | required BGP physical plan、变量投影、join order / stats hint | projected binding rows / count | 多 pattern BGP join、term-id merge join |
| `xpod_rdf.count` / `xpod_rdf.numeric_aggregate` | BGP physical plan、group keys、aggregate expression | grouped count / numeric aggregate rows | count、group count、duration/size 等数值聚合 |
| `xpod_rdf.result_cache_probe` | normalized query shape、graph scope、auth scope、facts `data_version` | cached rows / miss reason | repeated models query、列表页、Agent context |
| `xpod_rdf.result_cache_store` / materialized-result helpers | query shape、scope、version、projection、pagination、ttl/profile | cached/materialized row stream | 常用 dashboard / context query |
| `xpod_rdf.text_candidates` / `xpod_rdf.vector_candidates` / `xpod_rdf.score_fusion` | text/vector query、source scope、limit、score options | candidate ids + score | text/vector candidate generation 后再结构化 join |
...
- `bun run benchmark:rdf-models:pg` 已提供同 seed / 同 models case 的 PostgreSQL baseline gate，默认使用 PGlite 跑 `PostgresRdfEngine`，默认关闭 query result cache，执行前调用 `refreshDerivedIndexes()`，并把 `models-postgres-*.json` report 保存到 `.test-data/rdf-engine/`；也可用 `--driver=pg --connectionString=... --allowPgWrites` 跑真实 PostgreSQL，但只允许指向 disposable empty database，脚本会在写入前拒绝非空 RDF facts。任何 plan mismatch、seed 未达到目标规模或 derived stats 未同步都会让命令退出非 0。2026-06-04 small PGlite gate 中 seed 为 `114` quads，19 个 scan case 和 8 个 query case 均 plan matched，`queued run priority numeric aggregate` 走 `PostgresRdf3xJoinAggregate`，`message score by thread numeric aggregate` 走 `PostgresRdf3xGroupAggregate`，未落回 `PostgresFactsQuery`。同日真实 PG disposable medium gate 中 seed 为 `10066` quads，22 个 scan case 和 8 个 query case 均 plan matched，storage total/facts ratio 为 `1.39x`；`refreshDerivedIndexes()` 会同步 `ANALYZE` facts / RDF-3X stats 表，benchmark 默认 `warmupIterations=1`，避免把冷启动 planner stats 噪声计入稳态 p95。当前 warm steady-state 中 `latest message by thread query` 为 `17 ms`，`message join count distinct` 为 `8 ms`，PG baseline 可作为 cloud 默认正确性和稳态性能底座；冷启动首轮仍作为独立观测项保留。
