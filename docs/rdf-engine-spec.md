...
| Cache quota / TTL / eviction | result/materialized cache 有 TTL、max entries、payload bytes quota、PG table/index bytes 和 payload bytes stats；template cache 已补 max entries、idle TTL、eviction count 和 in-memory bytes 估算，`storageStats().derivedBytes` 汇总三类 cache | 后续补统一 derived cache quota、按 scope/facts version 的共享 bytes 预算，以及跨三类 cache 的统一淘汰观测 | cache 压测后不会无限增长；不同 scope 的淘汰互不污染；template TTL 过期后不会继续命中；result/materialized payload quota 超限后不会留下可命中的 cache row |
...
2. Cache lifecycle：第一步已完成结构化 access scope 与 PG result cache exact
   ...
   payload bytes quota 已完成第一版，template cache 已纳入 bytes 估算。
