...
- `MixDataAccessor.executeSparqlUpdate(...)` 已补上 embedded 文件权威路径：当目标是 `.ttl` / `.jsonld` by-line RDF 文档，并且 UPDATE 属于 `INSERT DATA` / `DELETE DATA` / `DELETE WHERE` / 安全 `DELETE/INSERT WHERE` / 安全 `INSERT WHERE` / 安全 `WITH` scoped update / default `USING` update 的单写目标 named graph delta 时，先读取需要参与 WHERE 的本地 RDF authority files，patch 目标 RDF authority file，再重建目标结构化 RDF index；其中 query-backed update 的 local WHERE bindings 复用 embedded `SolidRdfEngine.query(...)` 计算，所以已覆盖安全的 `FILTER` / `VALUES` / `OPTIONAL` / 受控 `UNION` / 受控 anti-join / semi-join / 固定长度 property path 子集。多 default `USING` 已支持读取多个本地 by-line RDF graph，但写入模板仍必须落在当前目标 document graph；多写目标 patch 或无法安全映射到本地 by-line graph document 的 shape 继续进入 compatibility accessor，并在回退后刷新本地 RDF mirror。
...
- Embedded index delta 已有 operation 级 `applyDelta(...)` 事务边界；文件权威层已支持多 default `USING` 读取多个本地 RDF authority files 并只 patch 当前目标文件。下一步继续扩大复杂 update 覆盖，例如更多 FILTER 表达式、更多 named graph shape 的安全映射评估，以及真正多写目标 patch 的事务/回退边界。无法安全映射的 shape 必须保留明确 fallback/错误和指标。
- 复杂 update 逐步消灭全量重写。
