...
第一版 target 可以用 graph/source 先缩小 source set，再在每个 source scope 内跑三元组
permutation scan；只有 benchmark 证明跨 graph 查询需要全局 quad permutation 时再扩展。
...
- graph-scoped BGP join 已开始走 RDF-3X membership source：当 pattern 带 exact graph 或 graph prefix 时，`Rdf3xTripleIndex` 不再固定先扫三元组 permutation 再回连 membership，而是直接以 `rdf3x_triple_membership` 作为该 pattern 的 source，使业务里常见的 `.data/chat/`、`.data/task/` 前缀能先收窄候选。graph prefix 先经 `rdf3x_stat_g` 图投影表收缩到真实 graph id，再进入 membership，避免 `rdf_terms` 里同前缀的 message subject IRI 参与候选。2026-05-27 medium models benchmark（`bun run benchmark:rdf-models -- --scale=medium --iterations=5`）里，所有 message BGP benchmark 均带 `.data/chat/default/` graph prefix：`latest message by thread local query` 的 RDF-3X p95 为 `84ms` vs term-id baseline `2155ms`，`message join count distinct` 为 `105ms` vs `2115ms`；`next queued run` / `run steps` 等小结果集保持 `1-2ms`。`rdf3x` profile 派生空间约 `2.58 MB`，`total/facts` 仍约 `1.40x`。
