...
- Embedded index delta 已有 operation 级 `applyDelta(...)` 事务边界；下一步继续扩大复杂 update 覆盖，例如更多 FILTER 表达式、更多 named graph shape 的安全映射评估，以及文件权威层多文档 patch 的事务/回退边界。无法安全映射的 shape 必须保留明确 fallback/错误和指标。
- 复杂 update 逐步消灭全量重写。
