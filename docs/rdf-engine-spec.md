...
索引补建不是 query-time adaptive indexing。Xpod 只支持代码定义好的 index profile：
`baseline`、`rdf3x`、未来的 `text` / `vector` / result-cache profile。启用某个
profile 时可以在启动、迁移或维护任务里创建缺失表和缺失 SQL index，然后按 facts
`data_version` 后台 backfill；query planner 只能在已有 profile 内选择索引和 join
顺序，不能因为某个查询临时在线新增一套物化结构。这样可以避免首个查询承担建索引
成本，也避免 cloud 多实例同时建索引导致锁竞争和不可预测的磁盘放大。
...
- RDF-3X 派生 stats 刷新：比较 `rdf_index_metadata.data_version` 和
  ...
  `Rdf3xIndex.rebuildFromCurrentQuads()`；查询前同步刷新只能作为兜底，不能成为
  大 Pod 的常态热路径。
