...
- 私有 `xpod-pro` 当前提供 Cloud PostgreSQL QLever 与 PG-native 模块；未来可承载其他通过公开稳定边界独立安装的商业差异化模块，但不拥有或削弱公开 Cloud 产品形态。
