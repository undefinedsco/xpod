...
| Cache quota / TTL / eviction | result/materialized cache 有 TTL/max entries 和 PG table/index bytes；template cache 已补 max entries、idle TTL、eviction count 和 in-memory bytes 估算，`storageStats().derivedBytes` 汇总三类 cache | 后续补统一 derived cache quota、按 scope/facts version 的 bytes 预算，以及 template/materialized 的统一淘汰观测 | cache 压测后不会无限增长；不同 scope 的淘汰互不污染；template TTL 过期后不会继续命中 |
