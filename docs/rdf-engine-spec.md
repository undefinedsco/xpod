...
| Explain / observability | 第一版已落地到 PG query metrics：`metrics.explain` 结构化输出 engine、facts version、derived profile、template/result/materialized cache 状态、结构化 access scope、acceleration/fallback 摘要、planner histogram hints、runtime 扫描/返回行数、RDF-3X stale stats 和 slow-query 诊断；`storageStats().rdf3x` 暴露 facts / RDF-3X facts version、refresh lag 和 synced boolean；`storageStats().slowQueries` 暴露 bounded process-local 最近慢查询 ring，记录 query/cache key、selected path、reason、runtime、stale stats、cache scope 摘要和 acceleration 摘要，不写入 Pod/RDF durable 状态；原有 plan 字符串继续保留给 benchmark gate | 后续把 slow-query stats 接到运维面板，并把 histogram / scan rows 真正接入 cost-based cutover | 慢查询报告可直接定位 fallback / cache miss / stale stats / refresh lag / 扫描放大；`/v1/rdf/stats` 可以直接读取最近慢查询快照 |
