...
| Cache quota / TTL / eviction | result/materialized cache 有 TTL、max entries、payload bytes quota、PG table/index bytes 和 payload bytes stats；template cache 已补 max entries、idle TTL、eviction count 和 in-memory bytes 估算，`storageStats().derivedBytes` 汇总三类 cache；`storageStats().derivedCache` 暴露三类 cache 的统一 bytes 占用、可选 `derivedCacheMaxBytes` 总预算，以及可选 `derivedCacheScopeMaxBytes` scope/facts-version 预算和最大 scope 占用 | 后续补跨三类 cache 的统一 eviction cause / pressure 观测 | cache 压测后不会无限增长；不同 scope 的淘汰互不污染；template TTL 过期后不会继续命中；result/materialized payload quota、scope quota 或统一 derived cache quota 超限后不会留下可命中的 cache row |
...
7. Incremental stats：PG 与 SQLite/file-backed dirty projection refresh 第一版已完成；后续补
   source-level 后台维护任务和更大数据量 benchmark。histogram、explain 随生产慢查询逐步补齐。
