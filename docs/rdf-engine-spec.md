...
| `Rdf3xTripleIndex` | 当前内部 shadow：从 `rdf_quads` 重建独立的 RDF 三元组六排列、projection stats 和 graph/source membership；不作为公开 backend selector。 |
...
- `Rdf3xTripleIndex` 是 first embedded slice：已覆盖 RDF-3X 数据布局、projection stats、permutation scan、基于 bound-slot fanout 的 connected BGP join order、term merge join、受控 index-only join，以及受控 single-pattern scan / count、同 pattern tuple VALUES scan、required BGP tuple VALUES join、OPTIONAL / UNION / dependent group 内部 BGP join、join count / basic numeric aggregate / grouped count / grouped numeric aggregate primary path；大多数 models 查询带 exact graph 或 graph prefix，因此这类 shape 在 scan/count/join 中优先以 membership source 收窄候选，而不是先扫三元组 permutation 再后置过滤 graph；派生排列表、统计表和 membership 表使用 SQLite `WITHOUT ROWID` 避免主键 autoindex 复制空间；文件型 `SolidRdfEngine` 标准配置会自动把它接成 selective primary，仍保留 `RdfQuadIndex` 作为迁移、benchmark 和 fallback。
- `SolidRdfEngine` 已接入内部 `derivedIndexProfile`：`baseline` 只保留事实层 term-id quad path，`rdf3x` 才会物化 `Rdf3xTripleIndex`。文件型 `index: { path }` 标准配置默认进入 `rdf3x` profile 并启用 selective primary；`:memory:` 和外部传入的 `RdfQuadIndex` 实例不会隐式创建第二个连接，仍可用显式 `rdf3xIndex + rdf3xPrimary` 进入 primary。query 只有在 RDF-3X 当前可表达的 single-pattern scan/count 或 required BGP（可含无 `UNDEF` 且所有变量均由 required BGP 绑定的 tuple VALUES；pattern 只含 exact term、graph prefix 和 object range）时，才把 scan / count / join / join count / basic numeric aggregate / grouped count / grouped numeric aggregate 下推到 `Rdf3xTripleIndex`。object range 会对 typed numeric literal 走 numeric 语义，对其他 term 走 lexical 语义。当前 index-only 只用于 `DISTINCT` term projection、无 graph 变量/graph 约束、无 pagination count 的 join；这种 shape 的 named graph multiplicity 对最终 term 集合无影响，所以可直接从三元组 permutation 表执行，其他 shape 仍回到 membership join。OPTIONAL / UNION / dependent join 仍由 local query layer 保持控制流语义，但其内部无 group-local `VALUES` 的多 pattern BGP 可走 RDF-3X join。未覆盖 shape 自动保持 `RdfQuadIndex` fallback，不暴露 backend selector。
...
- 存储不能按 “Hexastore + RDF-3X + QLever 全部常驻叠满” 理解。Pod RDF facts 只有一份权威数据；六排列、projection stats、result table、text/vector/cache 都是按当前 engine profile 选择性物化的派生索引，可删除、可重建、可按 local/cloud 资源预算关闭或延迟构建。
...
`Rdf3xTripleIndex` 在 `rdf3x_metadata` 记录自身已同步的 `facts_data_version`，
作为 primary 前要按版本刷新；已同步的同库索引在新 engine 实例启动后可以直接复用。
这样 CSS 兼容写路径、SPARQL update、direct engine put 和未来 sync journal 都能共享
...
- derived space：RDF-3X 六排列、projection stats、membership、未来 result table / cache / text-vector 辅助结构，都是可删除可重建的派生数据。
...
`SolidRdfEngine` 当前 v0 的 embedded baseline。`RdfLocalQueryEngine` 不是在
...
- 真正进入 RDF-3X 阶段时，要么实现独立的内部索引/planner 模块，要么把当前模块重构到上述结构；不能继续在 Hexastore-like quads 上堆功能后
  宣称已经完成 RDF-3X。

当前 first slice 是 `Rdf3xTripleIndex`：从现有 `rdf_quads` 重建独立 RDF-3X 数据，
文件型 `SolidRdfEngine` 标准配置会自动进入 selective primary；未覆盖 shape 继续 fallback 到 `RdfQuadIndex`，不接公开 backend selector。
...
第一版 target 把 exact graph / graph prefix 当成业务主路径：先用 graph/source membership
缩小 source set，再在 source scope 内执行 term 条件、range、排序和分页；没有 graph 约束时
才优先走三元组 permutation scan。只有 benchmark 证明跨 graph 查询需要全局 quad permutation
时再扩展。
SQLite embedded 形态下，RDF-3X 派生表必须使用 `WITHOUT ROWID`，让 permutation/stat
表的 primary-key btree 直接成为物理表，不再额外生成 `sqlite_autoindex_rdf3x_*`；
`rdf3x_triple_membership` 的 `(graph, subject, predicate, object)` primary key
也直接承担 GSPO membership lookup，避免再维护重复的 `rdf3x_membership_gspo`
二级索引。旧 rowid 版派生表可以丢弃并从 facts 重建。
...
- `RdfLocalQueryEngine` 已支持 `rdf3xPrimary` selective primary，文件型 `SolidRdfEngine` 标准配置会通过 `derivedIndexProfile: "rdf3x"` 自动启用；`derivedIndexProfile: "baseline"` 会保持同库事实层路径且不物化 RDF-3X 派生表，测试/外部实例路径也可显式打开：在 single-pattern scan/count 或 required BGP shape 被 `Rdf3xTripleIndex` 完整覆盖时，直接走 RDF-3X permutation/membership scan/join，并在 plan 中暴露 `Rdf3xPrimaryScan(...)`、`Rdf3xPrimaryCount(...)`、`Rdf3xJoinBGP(...)` / `Rdf3xPrimaryJoin...`；RDF-3X join order 会先用 projection / membership stats 选择最窄起点，再每步优先接入与已绑定变量连通的 pattern，并用 `count(pattern) / countDistinctTuple(pattern, boundSlots)` 估算 connected 候选 fanout，避免窄但不相关的 pattern 或高 fanout pattern 提前造成 cross product；连通 term equality 会落到 permutation `JOIN ... ON ...` 并在 plan 中标记 `Rdf3xMergeJoin(...)`，graph equality 仍在 membership 条件中保持 named graph 语义；typed numeric literal range 与 lexical object range 都可进入 RDF-3X scan/join；`DISTINCT` term projection 在无 graph 变量/graph 约束、无 limit/offset 的安全子集里会标记 `Rdf3xIndexOnlyJoin` 并跳过 `rdf3x_triple_membership`，直接利用 permutation 表的唯一三元组集合执行；同 pattern tuple VALUES scan 可用 `TupleValuesJoin(...)` 下推，required BGP 中无 `UNDEF` 且所有变量均由 required pattern 绑定的 tuple VALUES 可用 `Rdf3xJoinTupleValues(...)` 下推，join count、basic numeric aggregate、grouped count 和 grouped numeric aggregate 也可分别走 `Rdf3xJoinCount(...)` / `Rdf3xJoinAggregate(...)` / `Rdf3xJoinGroupCount(...)` / `Rdf3xJoinGroupAggregate(...)`。OPTIONAL / UNION / dependent group 仍在 local layer 维持 left join / branch / semi-join / anti-join 语义，但内部无 group-local `VALUES` 的 connected BGP pattern list 可批量走 `Rdf3xJoinBGP(...)`，避免退回逐 pattern scan。不支持的 FILTER、search 等 shape 不尝试半执行，继续落回 `RdfQuadIndex` fallback。
- `Rdf3xTripleIndex` 的 SQLite schema 已改为 compact derived schema：permutation/stat/membership/metadata 表均为 `WITHOUT ROWID`，并移除与 membership primary key 重复的 `rdf3x_membership_gspo` 索引；旧 rowid 派生表打开时会被丢弃并从 facts 重建。medium models benchmark 下，`rdf3x` profile 的派生空间从约 `4.76 MB` 降到约 `2.58 MB`，`total/facts` 从约 `1.74x` 降到约 `1.40x`，同时 `rdf3x shadow matched` 和 `rdf3x skipped join cases: 0` 保持不变。
- graph-scoped scan/count/count-distinct/join 已走 RDF-3X membership source：当 pattern 带 exact graph 或 graph prefix 时，`Rdf3xTripleIndex` 不再固定先扫三元组 permutation 再回连 membership，而是直接以 `rdf3x_triple_membership` 作为该 pattern 的 source，使业务里常见的 `.data/chat/`、`.data/task/`、provider/model/contact/favorite 前缀能先收窄候选。graph prefix 先经 `rdf3x_stat_g` 图投影表收缩到真实 graph id，再进入 membership，避免 `rdf_terms` 里同前缀的 subject/object IRI 参与候选。2026-05-27 medium models benchmark（`bun run benchmark:rdf-models -- --scale=medium --iterations=5`）里，22 个单 pattern case 中除 unsupported text search、exact id 和 provider relation lookup 外都走 `source-membership`；所有 LocalQuery BGP benchmark 都从 `source-membership` pattern 起步。`latest message by thread local query` 的 RDF-3X p95 为 `65ms` vs term-id baseline `1765ms`，`message join count distinct` 为 `72ms` vs `2290ms`；`next queued run` / `task materialization` 等小结果集保持 `1-3ms`。`rdf3x` profile 派生空间约 `2.58 MB`，`total/facts` 仍约 `1.40x`。
