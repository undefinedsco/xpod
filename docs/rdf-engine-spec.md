...
- graph-scoped BGP join 已开始走 RDF-3X membership source：当 pattern 带 exact graph 或 graph prefix 时，`Rdf3xTripleIndex` 不再固定先扫三元组 permutation 再回连 membership，而是直接以 `rdf3x_triple_membership` 作为该 pattern 的 source，使业务里常见的 `.data/chat/`、`.data/task/` 前缀能先收窄候选。2026-05-27 medium models benchmark（`bun run benchmark:rdf-models -- --scale=medium --iterations=5`）里，`latest message by thread local query` 的 RDF-3X p95 从旧实现约 `1801ms` 降到 `12ms`，相对同轮 term-id baseline `960ms` 快约 `98.8%`；`next queued run` / `run steps` 等小结果集保持 `1ms` 左右。当前 `message join count distinct` 仍是 `4ms` vs baseline `2ms`，说明 join aggregate/count 路径还不能宣称整体全胜。
