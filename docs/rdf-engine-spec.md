...
| Unsupported query boundary | 第一版已落地：server-owned Pod 默认不配置 compatibility fallback，`SolidRdfSparqlEngine` 对 unsupported query shape 返回 `UnsupportedSparqlQueryError`；无 fallback 时用户可见文案统一为 `Embedded SPARQL engine cannot execute <operation>: ... not supported by the embedded RDF engine`，不再泄漏 “fallback to compatibility engine” 这种过期执行心智；ACL/ACR 收紧 scope 下即使显式配置 fallback 也会拒绝不安全 fallback；`SubgraphSparqlHttpHandler` 把 unsupported shape 映射为 400，把禁用的 federation/SERVICE 映射为 403 | 后续补结构化 error code / capability hint，给 UI 和 Agent 更稳定的修正入口 | unsupported subquery/property-path 等 shape 在无 fallback 时返回 400 且错误文案说明 embedded engine 不支持；`onFallback` 不被调用、fallback metrics 不增长；restrictive ACL/ACR scope 不调用 fallback；SERVICE federation 不进入 fallback |
