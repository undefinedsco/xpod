...
| Bulk load + delayed index build | 第一版已落地：PG custom-index profile 支持启动时延迟创建 native permutation indexes，导入完成后显式 `ensurePgCustomIndexes()` 再进入 native cutover；PG facts 写入已把 term dictionary resolve、`rdf_quads` upsert 和 dirty projection queue 改成数组 staging / `UNNEST` bulk insert，并对 batch 内重复 quad 去重，避免 bulk seed 按 quad 逐条维护 native/custom index 或生成超长 `VALUES` SQL；PG models benchmark 支持 `concurrency` consistency gate，会用消息分页、任务调度 keyset、settings keyset、provider/model/credential ordered join 的串行结果作为基线，再并发复跑并校验 plan / row count / checksum / ordered checksum | 后续补真 PG COPY/staging-table 导入、大数据量 seed 完成后一次 refresh / ANALYZE，以及真实 PG large / high-concurrency benchmark | benchmark 可选择延迟 custom-index build；延迟期间 native-only operator 不 active、不会 500；ensure 后 6 个 custom permutation index 创建并恢复 native operator；bulk seed 只发固定批次数量的 term/quad insert，1300 quad smoke 仍保持单条 quad bulk insert；开启 `--concurrency=N` 时 report 会暴露 `concurrencyGate`，并发复跑不允许串结果或掉 plan；2026-06-08 PGlite medium/extreme baseline gate 在 UNNEST 写入路径下 19664 quads、2 个 scan case、10 个 query case plan matched，`rdf3x.syncedWithFacts=true`；million-scale gate 仍需重跑 |
...
6. Bulk load + delayed index build：PG custom-index 在 disposable benchmark / 迁移导入时的
   ...
   facts 写入内部使用数组 staging / `UNNEST` 批量 term upsert、quad upsert 和 dirty queue upsert，seed 完成后显式
   `ensurePgCustomIndexes()` 创建 native permutation index，再执行 `refreshDerivedIndexes()` /
   benchmark。PG models benchmark 已补 `--concurrency=N` 一致性 gate，默认不启用，启用后用串行基线对并发复跑做 plan / checksum 验收。后续再补真 PG COPY/staging-table 导入和 large / real-PG high-concurrency gate。
