...
这些来源处于不同抽象层级，不是多套并列 engine。公开运行时只有一个
`SolidRdfEngine` 行为契约：Hexastore 仅作历史索引参照，RDF-3X 提供 local/cloud
共同查询内核，可选原生执行层只能通过 vendor-neutral native SPARQL ABI 等位替换内部
执行路径。
...
| RDF-3X | RDF database engine | local/cloud 共同需要的排列索引、projection stats、merge join、join reorder、物理下推内核 | 替换 Comunica 主路径，而不是补在 Comunica 后面 |
| 可选原生执行层 | 内部 planner/executor | 复用同一 facts、权限和 native SPARQL ABI，不成为公开 backend | 不和 Comunica 协同执行 |
...
local 保持零额外服务、可移动、可重建的 embedded index；cloud 可由部署层提供与 ABI
兼容的实现，但公共仓库不依赖其构建、发布或运行时资产。
