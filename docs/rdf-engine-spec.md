...
3. Materialized result：PG 第一版表和 lifecycle 已完成；ChatKit thread history 读路径已回到
   ...
   thread-history 形状自动挂 materialized key。下一步优先覆盖 models 列表页、统计页和
   非 thread-history 的 Agent context materialized key。
