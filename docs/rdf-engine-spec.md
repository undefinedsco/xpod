...
5. Text / RDF / vector fusion：Agent Runtime 已有产品接入点：`RunContextRetriever`
   ...
   Inngest worker；默认产品 wiring 先启用 text/RDF 检索，vector retrieval 需要显式接入
   用户 Pod 中的 embedding provider 配置。后续补外部 vector backend 替换，以及默认部署
   text/vector 同步开关的产品入口。
