...
2. Cache lifecycle：第一步已完成结构化 access scope 与 PG result cache exact
   ...
   resource override index 已通过 WebACL/ACP target 解析接入；后续把 permissionVersion
   接进更细的失效判断，并把 template /
...
- query result cache 的 scope key 必须包含 principal、basePath、mode、authorization
  ...
  resource，再按 affected base path 与 graph scope 重叠删除相关 PG result/materialized cache。
  后续再把 permissionVersion 接入更细的失效判断。
