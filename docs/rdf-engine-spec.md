...
| Bulk load + delayed index build | 第一版已落地：PG custom-index profile 支持启动时延迟创建 native permutation indexes，导入完成后显式 `ensurePgCustomIndexes()` 再进入 native cutover；PG facts 写入已把 term dictionary resolve、`rdf_quads` upsert 和 dirty projection queue 改成数组 staging / `UNNEST` bulk insert，并对 batch 内重复 quad 去重，避免 bulk seed 按 quad 逐条维护 native/custom index 或生成超长 `VALUES` SQL | 后续补真 PG COPY/staging-table 导入、大数据量 seed 完成后一次 refresh / ANALYZE，以及 large/concurrency benchmark | benchmark 可选择延迟 custom-index build；延迟期间 native-only operator 不 active、不会 500；ensure 后 6 个 custom permutation index 创建并恢复 native operator；bulk seed 只发固定批次数量的 term/quad insert，1300 quad smoke 仍保持单条 quad bulk insert；2026-06-08 PGlite medium/extreme baseline gate 在 UNNEST 写入路径下 19664 quads、2 个 scan case、10 个 query case plan matched，`rdf3x.syncedWithFacts=true`；million-scale gate 仍需重跑 |
...
6. Bulk load + delayed index build：PG custom-index 在 disposable benchmark / 迁移导入时的
   ...
   benchmark。后续再补真 PG COPY/staging-table 导入和 large/concurrency gate。
