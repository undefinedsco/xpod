...
- `Rdf3xTripleIndex` 是 first shadow slice：只验证 RDF-3X 数据布局和统计，不改变主查询路径。
- `SolidRdfEngine` 已增加受控 `rdf3xPrimary` 开关：只在调用方显式打开、`rdf3xIndex` 已配置、且 query 是 RDF-3X 当前可表达的 required BGP（无 VALUES / OPTIONAL / UNION / dependent join / search source / aggregate，pattern 只含 exact term、graph prefix 和 numeric object range）时，`RdfLocalQueryEngine` 才把 join 下推到 `Rdf3xTripleIndex`。默认路径仍走 term-id `RdfQuadIndex`，未覆盖 shape 自动保持原路径，不暴露 backend selector。
...
当前 first slice 是 `Rdf3xTripleIndex`：从现有 `rdf_quads` 重建独立 shadow 数据，
只验证布局、统计和 scan 语义，不接公开配置、不替换 `SolidRdfEngine` 主路径。
...
- `RdfLocalQueryEngine` 已支持 opt-in `rdf3xPrimary` selective primary：在 required BGP shape 被 `Rdf3xTripleIndex.joinPatterns(...)` 完整覆盖时，直接走 RDF-3X permutation/membership join，并在 plan 中暴露 `Rdf3xJoinBGP(...)` / `Rdf3xPrimaryJoin...`。不支持的 FILTER、VALUES、OPTIONAL、UNION、search、aggregate 等 shape 不尝试半执行，继续落回默认 `RdfQuadIndex` 主路径。
