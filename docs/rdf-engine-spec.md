...
| Cache quota / TTL / eviction | result/materialized cache 有 TTL、max entries、payload bytes quota、PG table/index bytes 和 payload bytes stats；template cache 已补 max entries、idle TTL、eviction count 和 in-memory bytes 估算，`storageStats().derivedBytes` 汇总三类 cache；`storageStats().derivedCache` 暴露三类 cache 的统一 bytes 占用和可选 `derivedCacheMaxBytes` 总预算 | 后续补按 scope/facts version 的共享 bytes 预算，以及跨三类 cache 的统一 eviction cause / pressure 观测 | cache 压测后不会无限增长；不同 scope 的淘汰互不污染；template TTL 过期后不会继续命中；result/materialized payload quota 或统一 derived cache quota 超限后不会留下可命中的 cache row |
...
2. Cache lifecycle：第一步已完成结构化 access scope 与 PG result cache exact
   ...
   `derivedCacheMaxBytes` bytes guard 已完成第一版。
