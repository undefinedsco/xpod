...
| Ordered-page / keyset join | 第一版 benchmark gate 已落地：消息流 `createdAt < cursor + ORDER BY createdAt DESC + LIMIT`、任务调度 `nextRunAt` keyset continuation、settings `settingKey` continuation 都会要求 range/order/limit 保持在 SQL self-join / RDF-3X join 内；`pg-custom-index` 的 `join.required_bgp.order_page.topn.native` 已接入可选 ABI gate，当前只支持 ORDER BY 变量在 project 中的 required BGP，并要求 native `bgp_order_page(...)` 在 extension 内完成 value-order 排序分页；只有旧 `join.required_bgp.order_page.native` 时保留 `bgp_join(...)` 行流 + SQL `rdf_terms` wrapper；models extreme profile 已补 `extreme native exact graph ordered-page query`，用 `native-stress.ttl` exact graph 验收 native marker；2026-06-09 真实 PG17 baseline / hot / custom rerun 显示 exact-graph ordered-page wrapper p95 为 `18 ms` / `23 ms` / `14 ms`，公开 hot profile 不是 ordered-page cutover 依据；同日 `xpod-rdf-components` 已补十参数 `bgp_order_page` ABI 和 `rdf_terms.value` DESC smoke，但实现是 `bgp_join(...)` + `rdf_terms` ORDER/LIMIT 的 SPI compatibility wrapper；新增真实十参数 benchmark `two_pattern_bgp_value_order_page` 后，PG17 `bench-pg17-docker.sh` 显示 facts/btree p95 `5.355ms`，当前 `xpod_rdf_bgp_order_page_value_spi` p95 `7410.430ms`，shared hit blocks p95 `124` vs `35,358,553`，所以 top-N capability 仍不能由 extension 暴露；旧八参数 `two_pattern_bgp_order_page` 只测 term-id ordered-page，不可再作为 value-order top-N 证据；extension smoke 继续保持负向 capability guard | 后续把 extension wrapper 换成真正按 `rdf_terms.value` early-stop 的 C 层实现、补非 projected order variable 支持，以及 xpod models positive benefit cutover gate | benchmark 覆盖 ordered page correctness、稳定 cursor、任务调度 continuation、设置列表 continuation、p95 对比；mock extension gate 能命中 `PostgresRdfNativeCustomIndexBgpOrderPageTopN(...)`，旧 capability 只命中 wrapper，native capability 全缺时必须回退 RDF-3X ordered join；公开 hot profile 慢于 RDF-3X 时不能切默认，custom 也必须按 shape/cost gate |
