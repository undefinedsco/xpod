...
**测试类型门禁（同期另一会话补齐，本轮补最后一处）**：本轮开始时 `bun run typecheck:test` 在本机根本跑不起来 —— 根 `node_modules` 的 `@vitejs/plugin-react@5.2.0` 的 `index.d.ts` 用了 TS 5.6+ 的 `export { … as "module.exports" }` 语法，而本机 `tsc` 是 5.5.4（`TS1003`；`skipLibCheck` 不抑制 .d.ts 的**语法**错误），该 gate 也未进 CI。另一会话同期以 `c4ce42b7` / `88c9d462` / `16e3d4dd` 修掉依赖声明债并把 gate 接进 CI（新增 `types/vite-plugin-react.d.ts` 等）。两者合入后，本机只剩 `scripts/accept-network-tunnel.ts` 的 `import.meta.dir` 报错（Bun 扩展，此前只在文档里记为「既有缺口」）—— 但该脚本现在会被 gate 扫到，于是把 Bun 的两个 `import.meta` 扩展（`dir`/`main`）声明进会被自动纳入编译的 `types/bun/index.d.ts`：`bun run typecheck:test` 由 **exit 2 → exit 0**。
