...
Provide a planner and measurement layer after P0/P1/P2 physical sources are correct: make text, vector, RDF, path, and authorization constraints planner-visible, safely reorderable, and measurable through benchmarks.

Logical source/operator contracts and observable semantics remain stable. The optional native SPARQL ABI is the planner/executor interoperability layer; the `xpod_rdf` SQL ABI is the PostgreSQL adapter/operator layer. Capability sets, physical plans, and performance may differ. The normative boundary is defined by the parent overview.
...
- explicit score/ranking observability for text/vector candidate sources.
- benchmark gates for fusion workloads and serving-query regressions.

P3 excludes:

- replacing authority storage.
- exposing implementation-specific SPARQL extension syntax.
- violating the parent overview's planner integration boundary.
...
## Planner integration goals

Planner-visible sources enable integration without requiring a second authority index.
...
- optional native SPARQL ABI experiments only when implemented.
