...
Planner-produced `IndexScan` plans set `needed_slots` from the scan variables, and the materializer emits only those slots in QLever permutation order. Constant subject/predicate/object columns are therefore used for native scan filtering but are not returned as IdTable columns. A zero-variable scan is represented as a zero-width IdTable with row count preserved separately from the encoded id buffer.
