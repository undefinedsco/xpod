...
`XpodQleverPhysicalIndex` is the first native QLever-shaped lower access surface over this protocol. It exposes the query capability snapshot, single and batch dictionary lookup/resolution, QLever id encoding/decoding/comparison, term prefix ranges, and `permutation(...).estimate(...)` / `permutation(...).scan(...)` / `permutation(...).count(...)` / `permutation(...).distinct(...)` / `permutation(...).estimateDistinct(...)` using `PlannerRequestContext`; scan/count/distinct/estimateDistinct calls preserve the request snapshot, graph scope, source scope, access scope, cancellation, and backend capability guards. This is the seam that a patched or embedded QLever `Index`/`Permutation` path should call. It must not grow SPARQL planning, join planning, modifiers, or fusion policy.
