...
QLever-backed text scans are data-adapter calls, not QLever-native postings. When `QueryExecutionContext::xpodPhysicalIndex()` is present, `TextIndexScanForWord` and `TextIndexScanForEntity` must route estimates and materialization through the physical `TEXT_SEARCH` candidate source; missing `TEXT_SEARCH` or prefix-word materialization fails closed for now. Score-column projection is materialized from the physical candidate score as a QLever double value. Fixed-entity scans bind the fixed entity through the Xpod term dictionary and pass it as `required_entities`; variable-entity scans require candidate rows with `resource_term`. QLever's original `getWordPostingsForTerm` / `getEntityMentionsForWord` path is not an Xpod fallback; it is reachable only in standalone upstream QLever runs outside the PG-backed adapter path.
