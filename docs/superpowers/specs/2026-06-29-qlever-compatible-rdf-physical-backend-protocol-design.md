...
The adapter query request already carries snapshot, source/path scope, and access-scope context into the generated scan request, so future QLever-backed execution does not bypass Solid snapshot, path/subtree constraints, or ACL/ACR semantics.
