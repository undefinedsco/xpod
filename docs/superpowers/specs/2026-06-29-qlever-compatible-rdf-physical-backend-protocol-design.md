...
> Can Xpod's Cloud Enterprise PostgreSQL-backed RDF engine provide the physical data capabilities that a QLever-style executor needs, without creating a second RDF fact store?
...
- QLever full upstream build gate: [`scripts/check-qlever-full-engine-build.cjs`](../../../scripts/check-qlever-full-engine-build.cjs), including Darwin/Homebrew prefix defaults and stale/incompatible `pkg-config jemalloc` quarantine for real upstream executable-link probes.
- QLever real adapter build gate: [`scripts/check-qlever-real-adapter-build.cjs`](../../../scripts/check-qlever-real-adapter-build.cjs), which configures the patched upstream tree, extracts dependency include roots from `compile_commands.json`, and builds the QLever-enabled adapter against real upstream headers.
- QLever real runtime smoke gate: [`scripts/check-qlever-real-runtime.cjs`](../../../scripts/check-qlever-real-runtime.cjs), which builds the patched upstream `engine` plus the upstream libraries needed by the adapter smoke, builds the QLever-enabled adapter, links a small executable against real upstream planner/runtime objects, and verifies `SparqlParser -> QueryPlanner -> QueryExecutionTree::getResult(true) -> IndexScan::getLazyScan -> XpodQleverPhysicalIndex -> xpod_rdf_backend_v1`.
...
- Scan block metadata ABI: `xpod_rdf_scan_block_metadata` / `xpod_rdf_scan_block_metadata_fn` in [`native/postgres/rdf_protocol/include/xpod_rdf_physical_backend.h`](../../../native/postgres/rdf_protocol/include/xpod_rdf_physical_backend.h), surfaced through [`native/postgres/qlever_adapter/src/XpodPhysicalBackend.hpp`](../../../native/postgres/qlever_adapter/src/XpodPhysicalBackend.hpp)
...
- QLever physical index/permutation seam: [`native/postgres/qlever_adapter/src/XpodQleverPhysicalIndex.hpp`](../../../native/postgres/qlever_adapter/src/XpodQleverPhysicalIndex.hpp)
- QLever upstream `IndexScan::materializedIndexScan()` / `computeSizeEstimate()` / `getExactSize()` / `getScanSpecAndBlocks()` / `getLazyScan(...)` overlay patch: [`native/postgres/qlever_adapter/patches/qlever-indexscan-physical-lazy-scan.patch`](../../../native/postgres/qlever_adapter/patches/qlever-indexscan-physical-lazy-scan.patch)
- QLever upstream `TextIndexScanForWord::computeResult()` / `getCostEstimate()` / `getSizeEstimateBeforeLimit()` overlay patch: [`native/postgres/qlever_adapter/patches/qlever-text-indexscan-physical-text-search.patch`](../../../native/postgres/qlever_adapter/patches/qlever-text-indexscan-physical-text-search.patch)
- QLever upstream `TextIndexScanForEntity::computeResult()` / `getCostEstimate()` / `getSizeEstimateBeforeLimit()` / `knownEmptyResult()` overlay patch: [`native/postgres/qlever_adapter/patches/qlever-text-indexscan-entity-physical-text-search.patch`](../../../native/postgres/qlever_adapter/patches/qlever-text-indexscan-entity-physical-text-search.patch)
- QLever upstream `TextSearchQuery` fixed-entity overlay patch: [`native/postgres/qlever_adapter/patches/qlever-text-search-query-physical-fixed-entity.patch`](../../../native/postgres/qlever_adapter/patches/qlever-text-search-query-physical-fixed-entity.patch)
- QLever upstream `GroupByImpl::computeResult()` physical-index optimization guard patch: [`native/postgres/qlever_adapter/patches/qlever-groupby-physical-disable-native-optimization.patch`](../../../native/postgres/qlever_adapter/patches/qlever-groupby-physical-disable-native-optimization.patch)
- QLever `TextIndexScanForWord` / `TextIndexScanForEntity` physical source bridge: [`native/postgres/qlever_adapter/src/XpodQleverPhysicalTextIndexScanContextBridge.hpp`](../../../native/postgres/qlever_adapter/src/XpodQleverPhysicalTextIndexScanContextBridge.hpp)
- QLever upstream `QueryPlanner` physical-index guard patch: [`native/postgres/qlever_adapter/patches/qlever-queryplanner-physical-index.patch`](../../../native/postgres/qlever_adapter/patches/qlever-queryplanner-physical-index.patch)
- QLever upstream `QueryExecutionContext` physical-index injection patch: [`native/postgres/qlever_adapter/patches/qlever-queryexecutioncontext-physical-index.patch`](../../../native/postgres/qlever_adapter/patches/qlever-queryexecutioncontext-physical-index.patch)
- QLever upstream patch validator: [`scripts/check-qlever-upstream-patches.cjs`](../../../scripts/check-qlever-upstream-patches.cjs)
- QLever adapter-local `FRIEND_TEST` shim for production builds without gtest headers: [`native/postgres/qlever_adapter/src/gtest/gtest_prod.h`](../../../native/postgres/qlever_adapter/src/gtest/gtest_prod.h)
...
- QLever physical text-index scan context bridge test: [`tests/native/QleverPhysicalTextIndexScanContextBridge.test.ts`](../../../tests/native/QleverPhysicalTextIndexScanContextBridge.test.ts)
- QLever upstream text-index word overlay test: [`tests/native/QleverUpstreamTextIndexScanPatch.test.ts`](../../../tests/native/QleverUpstreamTextIndexScanPatch.test.ts)
- QLever upstream text-index entity overlay test: [`tests/native/QleverUpstreamTextEntityScanPatch.test.ts`](../../../tests/native/QleverUpstreamTextEntityScanPatch.test.ts)
...
The QLever physical permutation seam now has a lightweight `ScanSpecAndBlocks`-shaped adapter for `ScanSpecification` inputs. It preserves the selected permutation, mapped RDF slots, needed slots, and request scope; `scan(ScanSpecAndBlocks)` delegates to the same physical scan path as pattern scans, `getSizeEstimateForScan(...)` delegates to backend scan estimates and returns QLever-style lower/upper bounds, while `getResultSizeOfScan(...)` delegates to exact `countScan`.
The first upstream source overlay is intentionally narrow: it patches QLever `IndexScan::getLazyScan(...)` after QLever has already decided limit/offset and block prefiltering, and asks the injected `XpodQleverPhysicalIndex` for a lazy physical scan. In an Xpod-backed context, `UNSUPPORTED` from the physical seam is a contract failure, not permission to read QLever's native permutation files. QLever's original `permutation().lazyScan(...)` path remains only for pure upstream runs where no `xpodPhysicalIndex()` has been injected. This keeps block choice, lazy execution policy, and column subset handling in QLever-owned code while replacing the physical data source.
The QueryPlanner overlay is deliberately narrower than planner replacement. It does not choose joins or permutations for Xpod; it only prevents QLever's native "only two permutations loaded" guards from rejecting predicate-variable and three-variable scans when the `QueryExecutionContext` already carries an injected `XpodQleverPhysicalIndex`. This lets real upstream planning continue toward `IndexScan`, where the physical data access is redirected to Xpod. Without this guard patch, a PG-backed Xpod context with no QLever on-disk permutations can fail before the patched `IndexScan` seam is reached.
The QLever-enabled adapter build treats these overlays as part of the source-tree contract: `XPOD_QLEVER_SOURCE_DIR` must include patched `src/engine/IndexScan.cpp`, `src/engine/QueryPlanner.cpp`, text-index sources, and `QueryExecutionContext.h`; CMake fails with the patch-validator command when a hook is absent. This prevents a nominal QLever build from silently using QLever's own permutation or posting files instead of the Xpod physical protocol.

The lower C ABI now also exposes optional scan block metadata through `scan_block_metadata`. A backend can return block ids, first/last quad keys, row counts, sorted slots, total block count, and a metadata version for the same scoped `ScanRequest`. `XpodQleverPhysicalPermutation::getMetadataAndBlocks(...)` surfaces that metadata from a QLever-shaped scan spec, matching the upstream `Permutation::getMetadataAndBlocks(...)` seam closely enough for the next lazy-scan adapter step. `XpodQleverPhysicalPermutation::scanSelectedBlocks(...)` is the narrow consumer seam that passes a QLever-selected block list and metadata version into the same physical scan request; an empty selected-block list returns an empty IdTable and must not widen into a broad scan. This is still not lazy scan strategy: Xpod does not own block zipper joins, located-triples strategy, or block selection policy. Those remain QLever-owned and must consume this lower metadata instead of being reimplemented in Xpod planner code.
...
QLever-backed text scans are data-adapter calls, not QLever-native postings. When `QueryExecutionContext::xpodPhysicalIndex()` is present, `TextIndexScanForWord` and `TextIndexScanForEntity` must route estimates and materialization through the physical `TEXT_SEARCH` candidate source; missing `TEXT_SEARCH` or prefix-word materialization fails closed for now. Xpod-backed text scans require physical `TEXT_SEARCH`; QLever native postings are never an Xpod fallback. Score-column projection is materialized from the physical candidate score as a QLever double value. Fixed-entity scans do not validate against QLever's native vocabulary in an Xpod-backed context; `TextSearchQuery` preserves the fixed entity string, the physical text bridge resolves it through Xpod's batch term dictionary, and passes the resulting term key as `required_entities`. Variable-entity scans require candidate rows with `resource_term`. QLever's original `getWordPostingsForTerm` / `getEntityMentionsForWord` path is not an Xpod fallback; it is reachable only in standalone upstream QLever runs outside the PG-backed adapter path.
Text and vector candidate callbacks are materialized by a separate native C++ bridge into stable candidate rows. This keeps FTS/VEC candidate sources as protocol inputs to the future planner rather than TypeScript post-filters. Text and vector candidate sources also have QLever-style operation shells with estimate, execute, and profile behavior over `PhysicalBackend`, matching the `IndexScan` shell boundary without making candidates RDF triples. `BridgePhysicalPlan` can now carry text/vector candidate roots and execute them through a typed native result dispatcher that returns either RDF rows or candidate rows. Mixed text/RDF joins have a first native seam: a variable-entity `TextIndexScanForEntity` can feed candidate `ResourceTerm` keys into one RDF `IndexScan`, producing an RDF scan projection filtered by the candidate set and a profile tree with `HashJoin -> TextSearch + PermutationScan`. This is deliberately not yet a full QLever join projection that appends text columns. The public SPARQL facade can materialize supported text candidate roots into an RDF-compatible `IdTable` only when the declared candidate output columns can be encoded and dictionary-resolved; unsupported candidate roots remain fail-closed.
...
Text and vector candidate operation shells use the same Cloud Enterprise rule for feature bits: a missing capability callback, or an OK capability response without `TEXT_SEARCH` / `VECTOR_SEARCH`, fails closed before estimate/search callbacks. Local deployments do not need a degraded candidate-source runtime because they do not expose this adapter as a product feature. The upstream `TextIndexScanForWord` and `TextIndexScanForEntity` overlays follow the stricter Xpod-backed rule: once a physical index is injected, `TEXT_SEARCH` is required and QLever native postings are not a product fallback. This keeps FTS/vector availability in the native data contract that QLever-facing code can plan against.

`XpodQleverPhysicalIndex` is the first native QLever-shaped lower access surface over this protocol. It exposes the query capability snapshot, single and batch dictionary lookup/resolution, QLever id encoding/decoding/comparison, term prefix ranges, access/source scope resolution and estimates, scoped join-fanout estimates, histogram hints, text/vector candidate-source factories, QLever `ScanSpecification`-style col0/col1/col2 mapping into RDF slot patterns, and `permutation(...).estimate(...)` / `permutation(...).scan(pattern)` / `permutation(...).scan(scanSpecAndBlocks)` / `permutation(...).scanSelectedBlocks(...)` / `permutation(...).lazyScan(scanSpecAndBlocks, selectedBlocks)` / `permutation(...).lazyScanAll(scanSpecAndBlocks)` / `permutation(...).count(...)` / `permutation(...).distinct(...)` / `permutation(...).estimateDistinct(...)` / `permutation(...).getMetadataAndBlocks(...)` using `PlannerRequestContext`; scan/count/distinct/estimateDistinct/block-metadata, access/source scope, join-fanout, histogram, scan-spec, and candidate-source calls preserve the request snapshot, graph scope, source scope, access scope, cancellation, and backend capability guards. The upstream `IndexScan` overlay now also short-circuits construction-time metadata dependencies: when a physical index is injected, `IndexScan::getScanSpecAndBlocks()` requires the physical backend to scan and estimate the QLever `ScanSpecification`, returns the scan specification with empty QLever block metadata, `IndexScan::computeSizeEstimate()` asks the physical backend for the scoped estimate, and `IndexScan::determineMultiplicities()` estimates projected variable multiplicities through the physical `estimate_distinct` seam. If scan, estimate, or distinct-estimate capability is missing in an Xpod-backed context, the patch fails its contract instead of pretending empty metadata is valid or falling through to QLever's original permutation metadata. The same seam covers exact-size and non-lazy materialization: `IndexScan::getExactSize()` asks the physical backend `count_scan` through `getResultSizeOfScan(...)`, and `IndexScan::materializedIndexScan()` materializes an upstream `IdTable` from the Xpod physical backend. QLever's original permutation metadata/files remain reachable only for pure upstream runs with no injected physical index. The underlying scan request can carry selected block metadata for QLever `lazyScan(optBlocks)` integration; selected-block materialized and lower-lazy scans fail closed unless the backend declares `BLOCK_RESTRICTED_SCAN`. Empty selected-block scans return an empty result or empty block list instead of broadening. Non-prefiltered `getLazyScan(std::nullopt)` is different: when QLever provides neither selected blocks nor a usable `ScanSpecAndBlocks` block view, the upstream patch may explicitly allow `lazyScanAll(...)`, which runs the same scoped physical scan with `block_metadata_count = 0` and does not require `BLOCK_RESTRICTED_SCAN`. The lower lazy seam preserves backend scan callback batch boundaries as `IdTable` blocks, and `XpodQleverLazyScanBridge` can adapt those blocks into QLever's upstream `CompressedRelationReader::IdTableGeneratorInputRange` with `LazyScanMetadata` counters when the upstream `CompressedRelation` header is present. `XpodQleverPhysicalPermutation::lazyScanRange(...)` exposes that adapter directly from the physical seam, so a patched or embedded QLever `IndexScan::getLazyScan(...)` can call the native backend without re-materializing a single `Result`. `XpodQleverPhysicalIndexScanContextBridge` is the upstream patch helper for that call path: it retrieves the already-injected physical index from `QueryExecutionContext::xpodPhysicalIndex()`, maps QLever scan specifications or `ScanSpecAndBlocks::scanSpec_` through the physical permutation seam, converts selected `CompressedBlockMetadata` or the `ScanSpecAndBlocks::getBlockMetadataView()` non-prefiltered block view into Xpod block metadata, and delegates to physical materialized/lazy scan helpers. If no metadata is available, it broadens only when the caller proves the scan is not prefiltered; explicit empty selected blocks remain empty. It deliberately contains no planning, join, modifier, ranking, cache, limit/offset, or block-selection policy. Scan-spec graph filters that cannot yet be represented as Xpod graph scope fail closed instead of broadening results. This is the seam that a patched or embedded QLever `Index`/`Permutation` / text-index path should call. It must not grow SPARQL planning, join planning, modifiers, ranking, or fusion policy.
...
- Do not maintain QLever's on-disk RDF index beside Xpod-controlled RDF facts.
...
The QLever-enabled adapter has now been compiled as a standalone static library against a real patched upstream QLever source tree, using QLever's pinned dependency headers for Abseil, range-v3, uriparser, Boost, nlohmann-json, RE2, ICU, FSST, and CTRE. The adapter CMake exposes `XPOD_QLEVER_DEPENDENCY_INCLUDE_DIRS` for those upstream dependency include roots, and treats them as SYSTEM headers so adapter `-Werror` does not turn third-party warnings into Xpod build failures. `bun run check:qlever-real-adapter -- --qlever-source <patched-qlever-source> --qlever-build-dir <configured-qlever-build-dir>` is the reproducible gate for this standalone adapter build. This is a native build-time input only; it is not a product runtime configuration surface.

The local full-upstream probe has also compiled QLever's real `engine` target after applying the Xpod data-interface overlays plus the libc++ compatibility overlays. That probe is now reproducible through `bun run check:qlever-full-engine -- --qlever-source <patched-qlever-source> --build-dir <build-dir>`; the script first runs the patch validator, then configures upstream QLever with `XPOD_QLEVER_ADAPTER_ENABLE_QLEVER=1` and adapter/protocol include paths so `IndexScan.cpp` can include `XpodQleverPhysicalIndexScanContextBridge.hpp`. This proves the patched upstream engine can parse and compile the lower lazy-scan handoff to the Xpod physical seam; it still does not prove a product query path over a real PG-backed backend.
...
This compile gate is necessary but not sufficient. It proves that the Xpod native physical adapter currently parses real patched QLever headers and links its own static library against the expected upstream API surface. The planner, filter, and text-index overlays are part of the same compile contract: a QLever-enabled source tree must contain the `QueryPlanner` physical-index guard, the `Filter` bounded-term overlay, plus `TextIndexScanForWord` / `TextIndexScanForEntity` physical `TEXT_SEARCH` hooks, otherwise an Xpod-backed build would fail before `IndexScan`, evaluate bounded filters against the wrong vocabulary, or silently consult empty QLever-native postings. The runtime provider has the same guard: a constructible upstream `QueryExecutionContext` is exposed only when it can also receive the Xpod physical index hook.

The adapter also has a fixture runtime gate for the lower execution handoff: an upstream-shaped `QueryPlanner` fixture returns a `QueryExecutionTree` rooted at `IndexScan`, the bridge calls `QueryExecutionTree::getResult(true)`, the fake `IndexScan` calls its patched `getLazyScan(...)`, and that lazy scan reaches `XpodQleverPhysicalIndex` with QLever-selected block metadata before returning SPARQL JSON bindings. This proves the intended data-interface direction: QLever owns the planner/tree/lazy-result shape, while Xpod supplies the physical dictionary, scan, scope, and candidate-source backend.

The next gate after the fixture runtime is now reproducible as a real linked upstream runtime smoke:

```bash
bun run check:qlever-real-runtime -- \
  --qlever-source <patched-qlever-source> \
  --qlever-build-dir <qlever-build-dir> \
  --adapter-build-dir <adapter-build-dir> \
  --runtime-build-dir <runtime-build-dir>
```

This script builds the real patched upstream `engine`, then builds the upstream library targets required by the smoke (`parser`, `qlever`, `SortPerformanceEstimator`, `compilationInfo`), builds the QLever-enabled Xpod adapter, and links/runs a small executable against real upstream parser/planner/runtime objects. The smoke issues `SELECT * WHERE { ?s ?p ?o }` and proves the path `SparqlParser -> QueryPlanner -> QueryExecutionTree::getResult(true) -> IndexScan::getLazyScan -> XpodQleverPhysicalIndex -> xpod_rdf_backend_v1` without fake headers and without QLever's own on-disk RDF index. It also issues `SELECT ?s ?tail WHERE { ?s ?p ?o . ?o ?p2 ?tail }`, proving the public adapter can recover from upstream native-index stats assertions and execute a cross-slot RDF/RDF join through Xpod physical scans. It issues `SELECT DISTINCT ?s WHERE { ?s ?p ?o } ORDER BY ?s LIMIT 1`, proving the public adapter applies query-level SELECT modifiers for the direct upstream `QueryExecutionTree::getResult(true)` path before SPARQL JSON serialization. This is required because that lower tree API returns the internal execution table; QLever's server/export layer normally applies the public solution modifiers outside the raw tree result. It now also gates real upstream control-flow roots with `UNION`, `OPTIONAL`, and `MINUS`: `UNION` must preserve public projection plus distinct/order semantics, `OPTIONAL` must serialize QLever `UNDEF` cells as omitted SPARQL JSON bindings and still apply parsed `LIMIT/OFFSET` on the fallback operation-plan branch, and `MINUS` must bind fixed IRI terms through the Xpod physical dictionary before anti-join execution. It also gates `VALUES` as a physical candidate constraint with `SELECT ?s WHERE { VALUES ?s { <urn:s> <urn:o> } ?s ?p ?o } ORDER BY ?s`; the constants are looked up through Xpod's physical dictionary and materialized as QLever `IdTable` rows before joining with the BGP scan, so `VALUES` does not read from QLever-native vocab. It gates `BIND` execution with `SELECT ?s ?copy WHERE { ?s ?p ?o BIND(?s AS ?copy) } ORDER BY ?s`, proving that upstream QLever can append expression result columns over Xpod-backed scans and that Xpod only supplies the output-variable metadata needed for SPARQL JSON serialization. It gates bounded `FILTER` execution with `SELECT ?s ?o WHERE { ?s ?p ?o FILTER(?o != <urn:tail>) } ORDER BY ?s` and `SELECT ?s ?o WHERE { ?s ?p ?o FILTER(?o = <urn:o>) } ORDER BY ?s`; patched upstream `Filter::computeResult` first asks the Xpod physical backend to evaluate supported variable-vs-term equality/inequality using Xpod term ids, and only unsupported filter shapes fall through to upstream's original filter implementation. The former fail-closed bridge fallback for bounded term filters is intentionally removed from the direct planner path. It also gates `SELECT ?p (COUNT(?o) AS ?count) WHERE { ?s ?p ?o } GROUP BY ?p ORDER BY ?p`, `SELECT ?p (COUNT(?o) AS ?count) WHERE { ?s ?p ?o } GROUP BY ?p HAVING(COUNT(?o) > 1) ORDER BY ?p`, and `SELECT (COUNT(?s) AS ?count) WHERE { ?s ?p ?o }`, proving upstream QLever `GroupBy` can aggregate grouped, aggregate-filtered, and scalar count shapes over Xpod-backed physical scans when the execution context has an injected physical index. The bridge records aggregate alias output names for the direct upstream tree but marks them native-result-only, so the simplified compatibility operation executor cannot accidentally pretend to implement aggregate semantics. Aggregate `HAVING` / unsupported aggregate `Filter` descriptors are metadata-only pass-through when the child plan is native-result-only; QLever still owns expression evaluation. SPARQL JSON serialization now distinguishes QLever-generated inline numeric/boolean `Id` values from Xpod dictionary-backed term ids and emits aggregate counts as typed `xsd:integer` literals. It also gates `ASK { ?s ?p ?o }`, proving ASK query forms run over the Xpod physical scan and serialize a boolean response rather than being rejected as non-SELECT. It also issues `SELECT * WHERE { ?text ql:contains-word "topic" }` and `SELECT * WHERE { ?text ql:contains-word "topic" . ?text ql:contains-entity <urn:entity> }`, proving the text path `SparqlParser -> QueryPlanner/TextIndexScanForWord/TextIndexScanForEntity -> Xpod TEXT_SEARCH -> candidate-root SPARQL serialization`. Fixed entity text search resolves `<urn:entity>` through Xpod's physical dictionary before passing `required_entities`; it does not consult QLever-native vocab or text postings. The same rule applies to bounded filters and aggregate grouping keys: Xpod-backed direct execution must not use QLever-native vocabulary ids for constants or RDF terms.

This is still a seam gate, not a production PG dynamic-loader gate, not a local product feature, and not a broad SPARQL conformance suite. It uses an in-process `xpod_rdf_backend_v1` callback backend to prove that real upstream execution reaches the Xpod physical data protocol. Product data still comes only through the Xpod physical backend: QLever-native RDF permutations and native text postings remain empty in Xpod-backed mode and are not fallback data sources. Local deployment remains explicitly out of scope; the native adapter is packaged as Cloud Enterprise-only until this boundary is revisited.


### 2026-07-01 bounded Filter overlay update

The native adapter now carries `qlever-filter-physical-bounded-expression.patch` and `XpodQleverPhysicalFilterContextBridge.hpp`. The patch inserts a narrow early path into upstream `Filter::computeResult`: when the execution context has an Xpod physical index and the filter descriptor is a supported `?var = term`, `term = ?var`, `?var != term`, or `term != ?var` expression, the bridge resolves the constant through Xpod's physical term dictionary and filters the upstream `IdTable` against encoded Xpod/QLever ids. Missing constants produce empty equality results or all-row inequality results; unsupported shapes return `UNSUPPORTED` and let upstream QLever continue with its original filter implementation.

This update removes the previous direct-planner fail-closed guard for bridge-owned `EqualTerm` / `NotEqualTerm` modifiers. Real runtime smoke must therefore prove the path `SparqlParser -> QueryPlanner -> QueryExecutionTree::getResult(true) -> patched Filter -> Xpod physical term dictionary` for both IRI and literal bounded filters. The compatibility operation bridge remains available for unsupported direct-tree shapes, but it is no longer the default way to execute bounded filters once the patched upstream tree is available.

### 2026-07-01 GroupBy aggregate overlay update

The native adapter now carries `qlever-groupby-physical-disable-native-optimization.patch`. The patch guards upstream `GroupByImpl::computeResult`: when `QueryExecutionContext` has an injected Xpod physical index, QLever skips `computeOptimizedGroupByIfPossible()` because that shortcut depends on QLever-native permutation/stat stores that Xpod-backed contexts deliberately do not populate. Upstream `GroupBy` then consumes the normal child result table produced by patched `IndexScan` / joins over Xpod physical scans.

This is an execution-boundary fix, not a second aggregate executor. The compatibility operation bridge still only owns the older group-key dedup subset; aggregate aliases are recorded so direct upstream results can be serialized, but plans containing aggregate aliases are marked native-result-only and return `UNSUPPORTED` if routed to the fallback operation executor. Runtime smoke must cover `SELECT ?p (COUNT(?o) AS ?count) WHERE { ?s ?p ?o } GROUP BY ?p ORDER BY ?p`, `SELECT ?p (COUNT(?o) AS ?count) WHERE { ?s ?p ?o } GROUP BY ?p HAVING(COUNT(?o) > 1) ORDER BY ?p`, and `SELECT (COUNT(?s) AS ?count) WHERE { ?s ?p ?o }`, verifying RDF grouping keys, aggregate HAVING, and QLever-generated inline numeric count values. Inline QLever `Int` / `Double` / `Bool` values must serialize as typed literals; RDF terms still resolve through Xpod's physical dictionary. Aggregate HAVING is not reimplemented in Xpod: the operation-plan bridge only lets unsupported `Filter` metadata pass through when the child is native-result-only, keeping ordinary RDF filter fallback fail-closed.
