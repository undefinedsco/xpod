...
- QLever `TextIndexScanForWord` physical source bridge: [`native/postgres/qlever_adapter/src/XpodQleverPhysicalTextIndexScanContextBridge.hpp`](../../../native/postgres/qlever_adapter/src/XpodQleverPhysicalTextIndexScanContextBridge.hpp)
...
- QLever upstream text-index scan overlay test: [`tests/native/QleverUpstreamTextIndexScanPatch.test.ts`](../../../tests/native/QleverUpstreamTextIndexScanPatch.test.ts)
...
The first upstream source overlay is intentionally narrow: it patches QLever `IndexScan::getLazyScan(...)` after QLever has already decided limit/offset and block prefiltering, asks the injected `XpodQleverPhysicalIndex` for a lazy physical scan, and falls back to QLever's original `permutation().lazyScan(...)` only when the Xpod seam reports `UNSUPPORTED`. This keeps block choice, lazy execution policy, column subset handling, and fallback semantics in QLever-owned code while replacing the physical data source.
...
QLever-backed text scans are data-adapter calls, not QLever-native postings. When `QueryExecutionContext::xpodPhysicalIndex()` is present, `TextIndexScanForWord` must route estimates and materialization through the physical `TEXT_SEARCH` candidate source; missing `TEXT_SEARCH`, prefix-word materialization, or score-column projection fails closed for now. QLever's original `getWordPostingsForTerm` / `getEntityMentionsForWord` path is preserved only for pure upstream QLever runs with no injected Xpod physical index.
...
This compile gate is necessary but not sufficient. It proves that the Xpod native physical adapter currently parses real patched QLever headers and links its own static library against the expected upstream API surface. The text-index overlay is part of the same compile contract: a QLever-enabled source tree must contain the `TextIndexScanForWord` physical `TEXT_SEARCH` hook, otherwise an Xpod-backed build would silently consult empty QLever-native postings.
