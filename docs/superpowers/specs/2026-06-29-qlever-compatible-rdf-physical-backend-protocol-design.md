...
The QLever-enabled adapter build treats that overlay as part of the source-tree contract: `XPOD_QLEVER_SOURCE_DIR` must include a patched `src/engine/IndexScan.cpp`, and CMake fails with the patch-validator command when the hook is absent. This prevents a nominal QLever build from silently using QLever's own permutation files instead of the Xpod physical protocol.
...
This compile gate is necessary but not sufficient. It proves that the Xpod native physical adapter currently parses real patched QLever headers and links its own static library against the expected upstream API surface. The text-index overlays are part of the same compile contract: a QLever-enabled source tree must contain the `TextIndexScanForWord` and `TextIndexScanForEntity` physical `TEXT_SEARCH` hooks, otherwise an Xpod-backed build would silently consult empty QLever-native postings. The runtime provider has the same guard: a constructible upstream `QueryExecutionContext` is exposed only when it can also receive the Xpod physical index hook.
...
The remaining gap is full real-upstream execution. The current local probe compiles QLever's real upstream `engine` target and the standalone QLever-enabled adapter against real upstream headers/dependencies, and the provider can now construct an owned upstream-shaped execution context only when it can inject the Xpod physical index. The runtime query gate still uses upstream-shaped fixtures. The next correctness gate is a linked real-upstream execution binary that runs `QueryPlanner -> QueryExecutionTree::getResult(true) -> IndexScan::getLazyScan -> XpodQleverPhysicalIndex -> PG/RDF backend` without fake headers and without QLever's own on-disk RDF index.
