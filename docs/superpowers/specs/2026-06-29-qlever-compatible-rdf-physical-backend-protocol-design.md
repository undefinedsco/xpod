...
- QLever-shaped `ScanSpecAndBlocks` adapters may be lightweight while Xpod has no compressed block metadata. They may expose estimate/count/scan over the same scoped `ScanRequest`, but they must not pretend to support QLever lazy block filtering until the physical backend can return block metadata with correct snapshot and access-scope semantics.
