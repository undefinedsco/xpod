...
The parsed-query fallback can also project a `GRAPH ?g` scope for a single BGP scan by adding `XPOD_RDF_SLOT_GRAPH` to the scan projection and appending the graph variable after S/P/O in materializer order. Multi-triple `GRAPH ?g` fallback groups still fail closed until the fallback can express a subject+graph join key; fixed-IRI GRAPH groups remain exact graph scan constraints.
