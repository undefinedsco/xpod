...
This is an execution-boundary fix, not a second aggregate executor. The compatibility operation bridge still only owns the older group-key dedup subset; aggregate aliases are recorded so direct upstream results can be serialized, but plans containing aggregate aliases are marked native-result-only and return `UNSUPPORTED` if routed to the fallback operation executor. Runtime smoke must cover both `SELECT ?p (COUNT(?o) AS ?count) WHERE { ?s ?p ?o } GROUP BY ?p ORDER BY ?p` and `SELECT (COUNT(?s) AS ?count) WHERE { ?s ?p ?o }`, verifying RDF grouping keys and QLever-generated inline numeric count values. Inline QLever `Int` / `Double` / `Bool` values must serialize as typed literals; RDF terms still resolve through Xpod's physical dictionary.
