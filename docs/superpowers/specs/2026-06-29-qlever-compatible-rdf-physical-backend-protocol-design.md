...
The operation-to-plan bridge now handles the first real upstream operation shape: `IndexScan` is detected through the `Operation` base or a `QueryExecutionTree` root, its subject/predicate/object components and `Permutation` are converted to `BridgeQueryPlan`, and constants still bind through the native batch dictionary before execution. This starts replacing parser-only planning with planner-output planning while preserving the same Xpod physical scan executor.
