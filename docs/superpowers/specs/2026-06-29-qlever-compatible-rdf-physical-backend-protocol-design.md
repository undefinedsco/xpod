...
This is still a seam gate, not a production PG dynamic-loader gate and not a broad SPARQL conformance suite. It uses an in-process `xpod_rdf_backend_v1` callback backend to prove that real upstream execution reaches the Xpod physical data protocol. Product data still comes only through the Xpod physical backend: QLever-native RDF permutations and native text postings remain empty in Xpod-backed mode and are not fallback data sources.
