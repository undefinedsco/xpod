...
The adapter query request already carries snapshot, graph scope, source/path scope, and access-scope context into every generated RDF scan request, including fallback parsed plans and planner-context-derived scans, so future QLever-backed execution does not bypass Solid snapshot, graph, path/subtree, or ACL/ACR semantics.
The bridge request-context application now also carries request graph scope into primary, filter, and child RDF scan plans. Candidate source requests do not yet expose a graph-scope field, so text/vector graph filtering still belongs to the candidate-source protocol work rather than this RDF scan propagation seam.
