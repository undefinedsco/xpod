...
The remaining gap is full real-upstream execution. The current local probe compiles both the standalone adapter and QLever's real upstream `engine` target, and the provider can now construct an owned upstream-shaped execution context only when it can inject the Xpod physical index. The runtime query gate still uses upstream-shaped fixtures. The next correctness gate is a linked real-upstream execution binary that runs `QueryPlanner -> QueryExecutionTree::getResult(true) -> IndexScan::getLazyScan -> XpodQleverPhysicalIndex -> PG/RDF backend` without fake headers and without QLever's own on-disk RDF index.
