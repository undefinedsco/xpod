...
The lower C ABI now also exposes optional scan block metadata through `scan_block_metadata`. A backend can return block ids, first/last quad keys, row counts, sorted slots, total block count, and a metadata version for the same scoped `ScanRequest`. `XpodQleverPhysicalPermutation::getMetadataAndBlocks(...)` surfaces that metadata from a QLever-shaped scan spec, matching the upstream `Permutation::getMetadataAndBlocks(...)` seam closely enough for the next lazy-scan adapter step. This is a metadata seam only: Xpod still does not own lazy scan, block zipper joins, or located-triples strategy. Those remain QLever-owned and must consume this lower metadata instead of being reimplemented in Xpod planner code.
