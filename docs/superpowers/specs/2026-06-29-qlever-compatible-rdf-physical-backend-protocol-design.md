...
Text and vector candidate callbacks are materialized by a separate native C++ bridge into stable candidate rows. This keeps FTS/VEC candidate sources as protocol inputs to the future planner rather than TypeScript post-filters. The text candidate source also has a QLever-style operation shell with estimate, execute, and profile behavior over `PhysicalBackend`, matching the `IndexScan` shell boundary without making text results RDF triples.
