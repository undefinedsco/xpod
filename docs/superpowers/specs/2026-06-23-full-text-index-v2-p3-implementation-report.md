...
P3 acceptance-gate behavior is implemented for the scoped native-planner planner
boundary: text, vector, RDF, path, authorization, and VALUES sources are
planner-visible, costed, reorderable where covered, and product-scale benchmark
gated. This is still not a claim that Xpod embeds a native external native planner engine or
implements every external native planner optimizer feature.
...
itself a completion gate for a full native-planner planner because it can pass with
the postings text backend. The stricter `--productNativePlannerGate` is now
...
1. Native baseline artifact:
   `.test-data/rdf-engine/native-product-latest-20260629062731/baseline-20/models-postgres-2026-06-28T22-27-31-805Z-54733-33cb5599-a31c-417d-a5ec-96386793399e.json`.
2. native-planner planner artifact:
   `.test-data/rdf-engine/native-product-latest-20260629062731/hot-20-from-current-baseline-rerun/models-postgres-2026-06-28T22-47-33-252Z-64451-f6464e5c-a122-4c7b-8c4e-d4f97c2b7fbe.json`.
3. Gate command:

   ```bash
   bun scripts/assert-rdf-benchmark-report-gate.ts \
     --root=.test-data/rdf-engine/native-product-latest-20260629062731/hot-20-from-current-baseline-rerun \
     --productNativePlannerGate
   ```
...
- P3 currently exposes integrated-planner planner evidence and partial source integration; it is not a native external native planner backend.
- Source ordering is adaptive and marker-visible, with a bounded multi-step
  ...
  Planner markers now expose row cost, source base rows, variable-distinct
  estimates, and first-pass CPU/IO cost dimensions for each candidate. The
  bounded lookahead now uses source base rows plus variable distinct counts to
  price future joined-source fanout. It is still not a full native external native planner
  optimizer: calibrated CPU/IO weights, multi-column correlation statistics, and
  workload-tuned distribution modeling are not yet implemented.
- Text ranking in the native-planner product artifact uses PostgreSQL native FTS
...
- Vector retrieval is integrated as a PostgreSQL vector candidate source in the
  native-planner product artifact (`PostgresNativeVector(...)`).
