...
- Create `src/api/handlers/ReachabilityHandler.ts`: exposes `GET /v1/signal/nodes/:nodeId/routes` and generic `POST /v1/signal/nodes/:nodeId/sessions` with `kind` set to `p2p` or `relay`.
...
- `POST /v1/signal/nodes/:nodeId/sessions` with request `kind: "p2p" | "relay"`
...
- Shared API routes for `GET /v1/signal/nodes/:nodeId/routes` and `POST /v1/signal/nodes/:nodeId/sessions` with `kind`-based dispatch.
