...
| integrated-planner SPARQL+Text | word/entity occurrence 与 RDF join 的设计经验 | 只吸收 text/entity/RDF join 思想，不暴露 external native planner backend |
...
目标不是追通用 MTEB 或 external native planner benchmark，而是回答：
...
目标是让产品自己识别哪些 retrieval 场景最重要，并持续在这些指标上优化，而不是只追通用 IR/MTEB/external native planner benchmark。
