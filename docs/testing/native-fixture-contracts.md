...
- RDF Literal 夹具必须覆盖生产会写入的结构化内容，至少包含内嵌引号、反斜杠和 JSON；不得只用无转义的短字符串证明字面量解析正确。原生桥接应使用上游提供的结构化 Literal API，不能解析调试用 `toStringRepresentation()`。
- 原生查询/更新能力还必须有一条产品入口门禁：通过实际 UI/Repository 写入，再由实际查询消费者读取。协议级夹具证明 ABI 和语义，不能代替产品数据通路。
