...
- `pg-custom-index` 的 native scan limit early-stop 已有单测 gate：无显式 order 的 limited scan 会要求 `index.xpod_rdf_perm.scan_any.limit` active 并标记 `PostgresRdfNativeCustomIndexScanAnyLimit(...)`；ordered-page join 仍未作为 xpod models cutover gate。消息流 keyset page、任务调度 `nextRunAt` continuation 和 settings `settingKey` continuation 已作为 RDF-3X / PG SQL baseline benchmark gate；VALUES join 已完成受限形状接线并能命中 native operator，但当前 real PG extreme p95 略慢于 RDF-3X / btree baseline。
