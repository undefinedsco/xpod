...
| P0 | 真实 PG COPY stream bulk ingest | Partial | `PgPoolExecutor.copyFromRows()` 已为 real `pg` client 接入 COPY FROM STDIN staging path；PGlite / fake pg pool 不暴露 capability 时继续走 staging/UNNEST fallback。退出条件：用 disposable real PG 跑 medium/large seed，确认 term 与 quad staging 命中 COPY，且记录 ingest wall-clock。 |
