...
| P1 | Extension-level ordered top-N early-stop | Open | TS 侧已接入可选 `join.required_bgp.order_page.topn.native` capability 和 `xpod_rdf.bgp_order_page(...)` ABI，并有 mock extension gate；2026-06-09 `xpod-rdf-components` 已补十参数 `bgp_order_page(rdf_quads, rdf_terms, indexes, constants, variableSlots, outputSlots, orderSlots, orderDesc, limit, offset)` 兼容重载，PG17 smoke/recovery 覆盖 `rdf_terms.value` DESC page 语义，但实现仍是 SPI wrapper：先走 `bgp_join(...)` 行流，再外层 join `rdf_terms` ORDER/LIMIT，不是 native value-order early-stop。同日新增真实十参数 benchmark `two_pattern_bgp_value_order_page`，PG17 `bench-pg17-docker.sh` 显示 facts/btree p95 `5.355ms`，当前 `xpod_rdf_bgp_order_page_value_spi` p95 `7410.430ms`，shared hit blocks p95 `124` vs `35,358,553`；旧八参数 `two_pattern_bgp_order_page` 只测 term-id ordered-page，不可作为 value-order top-N 证据。因此 extension 仍不能暴露 `join.required_bgp.order_page.topn.native`；`xpod-rdf-components` smoke 保持负向 capability guard。需要换成真正能按 `rdf_terms.value` 做 early-stop 的 C 层设计并补 positive benefit gate 后才能 Done。 |
...
- `pg-custom-index` 的 native scan limit early-stop 已有单测 gate：无显式 order 的 limited scan 会要求 `index.xpod_rdf_perm.scan_any.limit` active 并标记 `PostgresRdfNativeCustomIndexScanAnyLimit(...)`；ordered-page join 已接入两级 native gate：`join.required_bgp.order_page.topn.native` 会调用 `xpod_rdf.bgp_order_page(...)` 并标记 `PostgresRdfNativeCustomIndexBgpOrderPageTopN(...)`，只有旧 `join.required_bgp.order_page.native` 时继续走 `bgp_join(...)` + 外层 order/page wrapper，两个 ordered-page capability 都缺失时回退 RDF-3X ordered join。消息流 keyset page、任务调度 `nextRunAt` continuation 和 settings `settingKey` continuation 已作为 RDF-3X / PG SQL baseline benchmark gate；2026-06-09 真实 PG17 baseline/hot/custom rerun 显示 exact-graph ordered-page wrapper p95 为 `18 ms` / `23 ms` / `14 ms`，custom wrapper 有收益；同日 `xpod-rdf-components` extension smoke/recovery 通过，十参数 `bgp_order_page` ABI 已能匹配 xpod-jobs 的 `rdf_terms` / `orderDesc` 调用并通过 value-order smoke，但它仍是 `bgp_join(...)` + `rdf_terms` ORDER/LIMIT 的 SPI compatibility wrapper，不是 native early-stop；新增真实十参数 benchmark `two_pattern_bgp_value_order_page` 后，PG17 `bench-pg17-docker.sh` 显示 facts/btree p95 `5.355ms`，当前 `xpod_rdf_bgp_order_page_value_spi` p95 `7410.430ms`，shared hit blocks p95 `124` vs `35,358,553`，所以 extension 侧必须保持不声明 `join.required_bgp.order_page.topn.native`；旧八参数 `two_pattern_bgp_order_page` 只测 term-id ordered-page，不可再作为 value-order top-N 证据。VALUES join 已完成受限形状接线并能命中 native operator，但当前 real PG extreme p95 仍有慢于 RDF-3X / btree baseline 的 case。
