...
| P1 | Extension-level ordered top-N early-stop | Open | TS 侧已接入可选 `join.required_bgp.order_page.topn.native` capability 和 `xpod_rdf.bgp_order_page(...)` ABI，并有 mock extension gate；仍需要真实 `xpod_rdf` C extension 实现和 real PG benefit gate 后才能 Done。 |
