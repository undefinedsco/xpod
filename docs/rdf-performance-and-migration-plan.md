...
| P1 | Extension-level ordered top-N early-stop | Done | TS 侧接入可选 `join.required_bgp.order_page.topn.native` capability 和 `xpod_rdf.bgp_order_page(...)` ABI，并把 top-N shape 收窄为单排序变量、ORDER BY 变量必须在 project 中；planner 会对排序变量做 order-aware permutation，例如 predicate 常量 + object 排序时选 `POSG`，避免绑定 order term 后仍落到 `PSOG` 扫描。2026-06-09 `xpod-rdf-components` 十参数 `bgp_order_page(rdf_quads, rdf_terms, indexes, constants, variableSlots, outputSlots, orderSlots, orderDesc, limit, offset)` 已从 SPI compatibility wrapper 改成 native term-candidate loop：按 `rdf_terms.value` 顺序分批取 term id，绑定排序 slot 后复用 native BGP 递归和 row offset/limit early-stop；PG17 smoke/recovery 改为正向要求 `join.required_bgp.order_page.topn.native`。真实十参数 benchmark `two_pattern_bgp_value_order_page` 在 PG17 `bench-pg17-docker.sh` 上显示 facts/btree p95 `5.531ms`，`xpod_rdf_bgp_order_page_value_native` p95 `5.393ms`；旧 SPI wrapper 证据为 p95 `7410.430ms`，已不再是当前路径。注意 shared hit blocks 仍高于 baseline（`124` vs `14,243`），所以该能力是 shape-gated custom extension operator，不作为 cloud 默认 profile 或“所有 ordered page 更快”的结论。 |
...
挂载 tmpfs。baseline / hot 容器不安装 `xpod_rdf`；custom 容器挂载
`/Users/ganlu/develop/xpod-rdf-components/pg-rdf-extension/xpod_rdf` 的
Linux ARM64 musl artifact，并通过 `CREATE EXTENSION xpod_rdf` 激活真实 native path。
`postgres:17-bookworm` 因缺 `libc.musl-aarch64.so.1` 不能加载该 artifact，因此这次
real-extension gate 使用 `postgres:17-alpine`。
...
- `pg-custom-index` 的 native scan limit early-stop 已有单测 gate：无显式 order 的 limited scan 会要求 `index.xpod_rdf_perm.scan_any.limit` active 并标记 `PostgresRdfNativeCustomIndexScanAnyLimit(...)`；ordered-page join 已接入两级 native gate：`join.required_bgp.order_page.topn.native` 会调用 `xpod_rdf.bgp_order_page(...)` 并标记 `PostgresRdfNativeCustomIndexBgpOrderPageTopN(...)`，只有旧 `join.required_bgp.order_page.native` 时继续走 `bgp_join(...)` + 外层 order/page wrapper，两个 ordered-page capability 都缺失时回退 RDF-3X ordered join。消息流 keyset page、任务调度 `nextRunAt` continuation 和 settings `settingKey` continuation 已作为 RDF-3X / PG SQL baseline benchmark gate；2026-06-09 真实 PG17 baseline/hot/custom rerun 显示 exact-graph ordered-page wrapper p95 为 `18 ms` / `23 ms` / `14 ms`，custom wrapper 有收益；同日 `xpod-rdf-components` 十参数 `bgp_order_page` 已从旧 SPI compatibility wrapper 改为 native term-candidate loop，smoke/recovery 正向要求 `join.required_bgp.order_page.topn.native`；`two_pattern_bgp_value_order_page` benchmark 在 order-aware `POSG` 调用下显示 facts/btree p95 `5.531ms`，`xpod_rdf_bgp_order_page_value_native` p95 `5.393ms`，旧 `xpod_rdf_bgp_order_page_value_spi` p95 `7410.430ms`。注意 native shared hit blocks 仍高于 baseline（`124` vs `14,243`），所以它只作为单排序变量、projected order、order-aware permutation 的 custom extension operator，不作为默认 cloud cutover 或所有 ordered-page shape 的性能结论。VALUES join 已完成受限形状接线并能命中 native operator，但当前 real PG extreme p95 仍有慢于 RDF-3X / btree baseline 的 case。
