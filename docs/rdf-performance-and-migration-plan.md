...
| P1 | Graph-prefix grouped count / count distinct native cost gate | Open | 当前部分 graph-prefix native/custom case 仍慢于 RDF-3X / btree baseline，不能默认 cutover；mock extension 回归已锁住 graph-prefix COUNT DISTINCT 继续走 RDF-3X，并通过 planner explain 暴露 `cost-cutover-count-distinct-native-regression`。 |
