...
| P1 | Extension-level ordered top-N early-stop | Open | TS 侧已接入可选 `join.required_bgp.order_page.topn.native` capability 和 `xpod_rdf.bgp_order_page(...)` ABI，并有 mock extension gate；2026-06-09 检查 `xpod-rdf-components` 后确认真实 extension 仍是旧 `bgp_order_page(heap,indexes,constants,variableSlots,outputSlots,orderSlots,limit,offset)` ABI，缺 `rdf_terms` / `orderDesc`，且 PG17 Alpine `XPOD_RDF_BENCH_SCALE=5000 XPOD_RDF_BENCH_ITERATIONS=3` 显示 `two_pattern_bgp_order_page` native p95 `2.154ms` 慢于 facts/btree `0.283ms`，因此不能暴露 `topn.native`；`xpod-rdf-components` smoke 已增加负向 capability guard，旧 ABI 声明 `join.required_bgp.order_page.topn.native` 会直接失败。需要补齐真实 C ABI、DESC/order term 处理和 positive benefit gate 后才能 Done。 |
