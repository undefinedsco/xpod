...
| P0 | `large=1_000_000` / 更高并发真实 PG release gate | Open | 需要在 COPY 或等价 ingest path 后重跑；必须记录 warm steady-state p95、cold first query、planner stats refresh 和 storage ratio。`benchmark:rdf-models:pg:gate` 已提供 report 验收命令，但真实 1M+ report 证据尚未产出。 |
