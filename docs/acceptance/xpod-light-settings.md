...
| --- | --- | --- |
| Isolated real Xpod with two WebIDs/Pods | `tests/e2e/xpod-settings.spec.ts`; `tests/e2e/xpod-settings-rc.spec.ts`; acceptance item `solid-pod-isolation` | Local regression stays hermetic: it starts a temporary Xpod and performs both OIDC flows itself. RC acceptance instead consumes the two browser states generated from the deployed seed accounts, never starts another Xpod, proves the selected Pod URLs are distinct, and requires their hosts to use the Cloud-assigned managed Pod suffix derived from the RC IdP. |
...
| Models, Pod, Network, Services at desktop and narrow widths | `tests/e2e/xpod-settings.spec.ts`; `tests/e2e/xpod-settings-rc.spec.ts`; `tests/ui/settings-launch.test.ts` | `bun run ai-connections:accept:browser` runs against the temporary real Xpod. When the RC workflow has generated both browser states, the broader acceptance harness automatically selects the deployed-RC spec; otherwise it selects the hermetic spec. Identical browser gates execute once and their result is shared by the isolation and visual requirements. |
...
Command gates terminate on timeout deterministically: macOS/Linux runs use a detached process group, send `SIGTERM`, then escalate to `SIGKILL` after the kill grace window. Windows falls back to child-process termination. Result contracts validate the complete stdout before evidence is compacted; reports retain bounded, redacted head and tail diagnostics with an omission marker, so startup errors and final test statistics remain visible together. Timed-out results are reported as `timedOut:true` with a non-zero result.
