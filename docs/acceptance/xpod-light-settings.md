...
| API key persists for WebID A, is redacted from ordinary UI/API responses, and is absent for WebID B; the adapter writes explicit `plaintext-v1` Pod records | `tests/e2e/xpod-settings.spec.ts`; `tests/api/ai-gateway/ProviderConnectAdapters.test.ts`; `scripts/accept-xpod-settings.ts` command gate | The real-host E2E uses browser OIDC state, UI save/reload, then the protected Pod status API backed by trusted fetch + drizzle-solid to prove Alice's provider count increases while Bob's does not. The adapter regression proves `plaintext-v1` serialization and rejects legacy encrypted rows. SecretCell remains deferred until its Pod key ownership and rotation design is approved. |
