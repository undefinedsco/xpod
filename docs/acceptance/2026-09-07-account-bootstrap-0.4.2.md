...
priority for non-loopback pages to `ui/src/context/resolve-xpod-account-index.ts`, its dedicated
regression tests, and the 0.4.2 package version alignment. Loopback pages retain managed Local discovery even when their Local CSS emits
bootstrap. Invalid explicit bootstrap on public pages fails closed.
...
- Account and release contract tests: 13 files, 155 tests passed (release contracts and Account regression runs).
