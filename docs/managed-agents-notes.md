...
- `Run.workspace`、`Run.thread`、`RunStep.run` 都是 RDF URI 字段；`runId` 是本地查询字段，值仍是 Run 的 base-relative resource id，不是 RDF link。
...
- cloud 部署不单独包 bash，而是把完整的 pi Agent Loop 放入 sandboxed worker/process: model loop、工具调用、bash/read/write/edit 都在同一个受限执行边界内发生。
...
Chat/Task 发起 Run 时应该带明确的 `workspace` URI；没有显式 workspace 时不应悄悄回退到 Pod root 或服务端 `cwd`。workspace 在 Solid 模型中是 URI 字段，指向一个 Container:
...
稳定的 workspace 信息挂在对应 Container 的 `.meta`，不塞进 Thread/Run 的 metadata。Thread/Run 只记录 workspace URI。只有当 Pod Container 使用本机文件系统挂载时，`PiAgentRuntimeDriver` 才会把该 URI 映射到 `CSS_ROOT_FILE_PATH` 下的本地目录；MinIO/远程对象存储需要后续的 mount/sync/cache 层，不能假装已经是可沙盒的本地目录。`file://<authority>/...` 的 authority 表示能解析该路径的设备/runner 命名空间；当前 runner 不能解析时，Run 进入 `waiting_runner`。

执行策略:

- local: 直接以 Pod 根目录作为 cwd 启动 Agent Loop，不加沙盒，便于本地开发和低延迟交互。
- cloud: 必须把整个 Agent Loop 放进 sandboxed worker/process，而不是只拦截 `bash`。bash 仍然是可用工具；限制来自外层进程/OS sandbox 和 Pod root 挂载边界。
...
- `Run.thread`、`Run.workspace`、`Run.task` 都是 RDF URI 关系。
...
- `surfaceId` 是 command surface/channel 的归档槽位。ChatKit 外部协议仍叫 `chat_id`，内部 durable model 不再使用 `chatId` / `targetId` 表达这个含义。
...
`ownerType` 不需要在每个对象重复。若后续泛化 `Thread.owner`，使用 URI 归属即可；下游对象跟着 `thread` 或 `run` 走。`workspace` 是 URI 字段，不是 metadata，也不是 TS 类型层面的重点。
