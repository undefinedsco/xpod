...
private leading-prefix array custom-index scan ABI、`xpod_rdf_perm` custom index access
...
- native extension 只能声明已经真实接线的能力。当前 native provider 可声明
  ...
  custom-index scan + heap recheck；也可声明 `join.required_bgp`、`aggregate.count` 和
...
- `capabilityProviders` 按能力位记录实际来源。当前混合 profile 中，`cache.result` 可以来自
  ...
  可以来自 native `extension`；`index.xpod_rdf_perm.scan` 和
  `index.xpod_rdf_perm.scan_any` 只能来自 native `extension`；join / aggregate 在 native
  extension 安装后可以来自 `execute_plan_json`，没有 native extension 时仍来自 `engine-sql`。
