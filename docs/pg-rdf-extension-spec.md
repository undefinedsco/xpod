...
filter/order/limit/group 的 exact-id required BGP 子集可走 `bgp_join`。单 pattern scan
...
required BGP 行流；graph-prefix、term filter、range、order、limit、distinct、group 和 aggregate
仍回退。`xpod_rdf_perm` 仍尚未实现 block-level skip table、完整 custom index-aware join
...
- native extension 只能声明已经真实接线的能力。当前 native provider 可声明
  ...
  的受限 exact-id required-BGP row-stream ABI。probe object range filter/order 由
