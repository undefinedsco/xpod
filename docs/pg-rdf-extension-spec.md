...
subject-star count / count-distinct summary ABI、`xpod_rdf_perm` custom index access method
...
绕进 extension provider，而是留在 PG RDF-3X engine-sql path；只有受限 subject-star shape
会走 `subject_star_join`，同一受限 shape 的非分组 count / count-distinct 可进一步走
`subject_star_count`。单 pattern scan 的 range/text filter 仍回退到 PG RDF-3X engine-sql
...
- native extension 只能声明已经真实接线的能力。当前 native provider 可声明
  ...
  count-distinct summary ABI。probe object range filter/order 由 `PostgresRdfEngine` 的 outer
  SQL 完成，不进入 `subject_star_count`。`join.required_bgp`、`join.values`、
  `aggregate.count` 和 `aggregate.numeric` 仍可作为 engine-sql hot operator
...
- `capabilityProviders` 按能力位记录实际来源。当前混合 profile 中，`cache.result` 可以来自
  ...
  `index.xpod_rdf_perm.scan_any`、`join.subject_star` 和 `aggregate.subject_star_count`
  只能来自 native `extension`；
...
- 当前 native `0.1.0` 已实现最小 `scan.exact_graph` / `scan.graph_prefix` / `scan.term_in` ABI：
  ...
  `subject_star_join(...)` row stream + PG SQL aggregate。`execute_plan_json` 保留为 legacy ABI，但普通 required BGP join、group
