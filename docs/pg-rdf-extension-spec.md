...
leading equality/range prefix 估算 rows/pages，让 selective prefix 条件在正常 planner
下生成 `Index Scan`，包括 `subject_id = 42` 这类 PG 默认解析成 `bigint = integer`
的裸整数 literal。无序 append 会先降级
