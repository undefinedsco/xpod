...
path；subject-star probe object range 可由 `PostgresRdfEngine` 在 outer SQL 中 join
`rdf_terms` 执行 filter/order。`xpod_rdf_perm` 当前是
...
graph-prefix、term filter、range、distinct 和 native aggregate 仍回退。`xpod_rdf_perm` 仍尚未实现 block-level skip table、完整 custom index-aware join
