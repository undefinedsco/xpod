...
会走 `subject_star_join`。range/text filter 仍回退到 PG RDF-3X engine-sql path。`xpod_rdf_perm` 当前是
...
- native extension 只能声明已经真实接线的能力。当前 native provider 可声明
  ...
  `subject_star_join(...)` 的受限 subject-star seed/probe/recheck path。`join.required_bgp`、
