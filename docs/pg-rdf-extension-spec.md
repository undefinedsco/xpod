...
`xpod_rdf.count_quads(...)` 单 pattern scan ABI、`xpod_rdf_perm` custom index access
method 和 `xpod_rdf.term_id_ops` bigint opclass。`scan_quads` 当前只接收已经由
...
range/text filter、BGP join 和 aggregate 仍回退到 PG RDF-3X engine-sql path。`xpod_rdf_perm` 当前是
...
每个 index page 仍维护 key min/max range 和 page-local lower-bound seek；planner cost 已按
leading equality/range prefix 估算 rows/pages，让 bigint-typed selective prefix 条件在正常 planner
下生成 `Index Scan`。无序 append 会先降级
...
注意：`xpod_rdf.term_id_ops` 当前只声明 bigint search operators。查询生成器必须输出
bigint-typed term id 参数或显式 cast；裸整数字面量如 `subject_id = 42` 会解析成 PostgreSQL
内置 cross-type `bigint = integer` operator，planner 不会把它匹配到当前 opfamily。
...
- native extension 只能声明已经真实接线的能力。当前 native provider 可声明
  ...
  term-id / graph-prefix scan；不能声明 `join.required_bgp` 或 `aggregate.*`，这些能力仍由
  `engine-sql` provider 提供。
...
- `capabilityProviders` 按能力位记录实际来源。当前混合 profile 中，`cache.result` 可以来自
  ...
  可以来自 native `extension`；join / aggregate 在 native extension 替换前来自
  `engine-sql`。
...
- 当前 native `0.1.0` 已实现最小 `scan.exact_graph` / `scan.graph_prefix` / `scan.term_in` ABI：
  ...
  该 ABI 仍不是完整 native planner：排序、分页、DISTINCT、range/text filter、BGP join 和
  aggregate 继续使用 PG RDF-3X baseline / engine-sql hot path。
