...
required-BGP nested-loop prototype、`xpod_rdf_perm` custom index access method
...
`bgp_join` 作为输入 row stream，但 aggregate operator 本身还不是 native C executor；
非 subject-star graph-prefix、无法映射的 term filter、range、distinct 和 native aggregate 仍回退。`xpod_rdf_perm` 仍尚未实现 block-level skip table、完整 custom index-aware join
...
- native extension 只能声明已经真实接线的能力。当前 native provider 可声明
  ...
  `xpod_rdf.values_join(...)`。不能仅因 native extension 安装就代表 grouped / numeric aggregate 计算已进入 native
  C executor。
...
- 当前 native `0.1.0` 已实现最小 `scan.exact_graph` / `scan.graph_prefix` / `scan.term_in` ABI：
  ...
  BGP 变量 slot 后再递归扫描 custom index；ORDER/LIMIT/OFFSET 和 supported grouped / numeric
  aggregate 可在 native row stream 外层由 PG SQL 继续执行。`execute_plan_json` 保留为 legacy ABI，但 group aggregate 和 numeric
