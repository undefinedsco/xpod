...
filter 的 exact-id required BGP 子集可走 `bgp_join`；ORDER/LIMIT/OFFSET 可在
`bgp_join` row stream 外层 SQL 保守执行；supported grouped / numeric aggregate 也可以把
aggregate 输入 BGP 流切到 `bgp_join`；`VALUES` 约束变量也可以作为额外输出列接到
outer SQL tuple recheck，但 group / numeric aggregate 和 `VALUES` join 计算仍由外层 PG SQL 完成。
单 pattern scan
...
- native extension 只能声明已经真实接线的能力。当前 native provider 可声明
  ...
  的受限 exact-id required-BGP row-stream ABI，ORDER/LIMIT/OFFSET 和 `VALUES`
  tuple recheck 由 outer SQL 消化。
  ...
  `PostgresRdfEngine` 的 PG RDF-3X SQL join；supported native BGP 可以把 values 约束变量作为
  额外输出列，再接 outer SQL tuple recheck，但这不代表已经存在 native C `values_join`
  executor。不能仅因 native extension 安装就代表 grouped / numeric aggregate 计算已进入 native
  C executor。
...
- `capabilityProviders` 按能力位记录实际来源。当前混合 profile 中，`cache.result` 可以来自
  ...
  和 `join.required_bgp.native` 只能来自 native `extension`；
  VALUES join / aggregate 计算仍来自 outer `engine-sql`，直到 native custom executor
  通过 benchmark gate。
...
- 当前 native `0.1.0` 已实现最小 `scan.exact_graph` / `scan.graph_prefix` / `scan.term_in` ABI：
  ...
  8 个变量列；ORDER/LIMIT/OFFSET、`VALUES` tuple recheck 和 supported grouped / numeric
  aggregate 可在该 row stream 外层由 PG SQL 继续执行。`execute_plan_json` 保留为 legacy ABI，但 group aggregate 和 numeric
