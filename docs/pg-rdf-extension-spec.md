...
narrow native subject-star join prototype、`xpod_rdf_perm` custom index access method
...
ordinary required BGP join 和 count / numeric aggregate 当前不再通过 `execute_plan_json`
绕进 extension provider，而是留在 PG RDF-3X engine-sql path；只有受限 subject-star shape
会走 `subject_star_join`。单 pattern scan 的 range/text filter 仍回退到 PG RDF-3X engine-sql
...
这仍只是 scalar count fast path，不是完整 grouped/numeric custom aggregate executor。`xpod_rdf_perm` 仍尚未实现
...
- native extension 只能声明已经真实接线的能力。当前 native provider 可声明
  ...
  `subject_star_join(...)` 的受限 subject-star seed/probe/recheck path。probe object range
  filter/order 由 `PostgresRdfEngine` 的 outer SQL 完成，不新增 C ABI。`join.required_bgp`、
...
- `capabilityProviders` 按能力位记录实际来源。当前混合 profile 中，`cache.result` 可以来自
  ...
  `index.xpod_rdf_perm.scan_any` 和 `join.subject_star` 只能来自 native `extension`；
...
- 当前 native `0.1.0` 已实现最小 `scan.exact_graph` / `scan.graph_prefix` / `scan.term_in` ABI：
  ...
  在 outer SQL 中通过 `rdf_terms` 执行。同一受限 shape 的 count / count-distinct aggregate
  可以把 `subject_star_join(...)` 作为输入，再由 PG SQL aggregate
