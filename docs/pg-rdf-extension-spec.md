...
filter/group 的 exact-id required BGP 子集可走 `bgp_join`；ORDER/LIMIT/OFFSET 可在
`bgp_join` row stream 外层 SQL 保守执行。单 pattern scan
...
native index-level early stop / ordered stream；graph-prefix、term filter、range、distinct、
group 和 aggregate 仍回退。`xpod_rdf_perm` 仍尚未实现 block-level skip table、完整 custom index-aware join
...
BGP join、group aggregate 和 numeric aggregate 保持 direct PG RDF-3X SQL，避免
...
- native extension 只能声明已经真实接线的能力。当前 native provider 可声明
  ...
  executor。不能仅因 native extension 安装就代表 ordinary join / aggregate 已进入 native
  executor。
...
- `capabilityProviders` 按能力位记录实际来源。当前混合 profile 中，`cache.result` 可以来自
  ...
  ordinary join / VALUES join / aggregate 仍来自 `engine-sql`，直到 native custom executor
  通过 benchmark gate。
...
- 当前 native `0.1.0` 已实现最小 `scan.exact_graph` / `scan.graph_prefix` / `scan.term_in` ABI：
  ...
  可用时，非 subject-star、无 filter/order/limit/group 的 exact-id required BGP 子集会走
  `xpod_rdf.bgp_join(...)`，按 planner 给出的 join order 递归扫描 custom index 并返回最多
  8 个变量列。`execute_plan_json` 保留为 legacy ABI，但 group aggregate 和 numeric aggregate
  不再通过它执行；它们继续使用 PG RDF-3X baseline / engine-sql hot path。
