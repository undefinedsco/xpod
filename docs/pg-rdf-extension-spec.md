...
- 不让开源 cloud 版本依赖 native C artifact 才能启动或查询。开源 cloud 的默认
  acceleration profile 是 `pg-hot-operators`，没有 C extension 时由 `PostgresRdfEngine`
  的 engine-sql hot operators 和 schema-local SQL ABI 提供可用能力；native C /
  `pg-custom-index` 只作为可选加速包。
...
- `xpod_rdf` 可以作为单独的 native artifact 分发；缺失该 artifact 时，Xpod cloud 仍必须
  按 `pg-hot-operators` / PG RDF-3X baseline 正常运行。
- 开源仓库只需要保留 capability probe、engine-sql fallback、profile 配置和测试。C 源码、
  `.so`、`.control`、extension SQL 安装包可以放在独立私有仓库或授权制镜像层里。
- 官方 cloud / enterprise 镜像可以默认安装 native package；自部署开源 cloud 不安装时不能
  产生启动失败、Pod 读写失败或查询语义差异，只能少一部分 `extension` provider 的性能收益。

当前开发 checkout 中的源码位于 `native/postgres/xpod_rdf`，本地构建入口是：
...
private plan execution ABI、`xpod_rdf.perm_index_probe(regclass, bigint, bigint, bigint, bigint)`
private index probe ABI、`xpod_rdf.perm_index_scan(regclass, bigint, bigint, bigint, bigint)`
private leading-prefix custom-index scan ABI、`xpod_rdf.perm_index_scan_any(regclass, bigint[], bigint[], bigint[], bigint[])`
private leading-prefix array custom-index scan ABI、`xpod_rdf.perm_index_count(...)` /
`xpod_rdf.perm_index_count_any(...)` private leading-prefix custom-index scalar count ABI、`xpod_rdf.subject_star_join(...)`
...
required-BGP nested-loop prototype、`xpod_rdf.values_join(...)` narrow native VALUES+BGP prototype、
`xpod_rdf.bgp_count(...)` narrow native BGP count / count-distinct summary ABI、
`xpod_rdf.bgp_group_count(...)` narrow native BGP grouped count / count-distinct summary ABI、`xpod_rdf_perm` custom index access method
和 `xpod_rdf.term_id_ops` bigint opclass。`term_id_ops` 同时注册常见整数字面量
...
ordinary required BGP join 和 numeric aggregate 当前不再通过 `execute_plan_json`
绕进 extension provider；受限 subject-star shape 会走 `subject_star_join`，同一受限 shape
的非分组 count / count-distinct 可进一步走 `subject_star_count`。非 subject-star、无
filter 的 exact-id required BGP 子集可走 `bgp_join`；单个 tuple `VALUES` source
可走 `values_join`，在 native 端先把 VALUES 变量绑定到 BGP slot 再扫 custom index。
同一 exact-id required BGP 子集上的非分组 count / count-distinct 可走 `bgp_count`，
直接在 native row stream 上返回 summary；同一子集上的 count-only grouped aggregate 可走
`bgp_group_count`，由 native extension 返回 group keys + summary counts；当前 BGP
summary ABI 覆盖 1..4 个 exact-id pattern，所以单 pattern exact grouped count 也不再需要
先物化 `bgp_join` row stream。HAVING / ORDER / LIMIT 仍由 outer SQL 保守处理。
ORDER/LIMIT/OFFSET 可在 `bgp_join` / `values_join` row stream 外层 SQL 保守执行；
supported numeric aggregate 也可以把 aggregate 输入 BGP 流切到 native row stream，
但 numeric aggregate 计算仍由外层 PG SQL 完成；多源或暂不支持的 VALUES 形态仍回退
outer SQL tuple recheck。
单 pattern scan
的 range/text filter 仍回退到 PG RDF-3X engine-sql
path；subject-star probe object range 和可映射 term filter 可由 `PostgresRdfEngine`
在 outer SQL 中 join `rdf_terms` 执行 recheck/filter/order。对 native BGP summary 不适用的
single-pattern graph-prefix grouped aggregate，`PostgresRdfEngine` 可先用
`xpod_rdf.perm_index_scan(...) + heap recheck` 生成 custom-index 输入流，再由 outer SQL
执行 grouped aggregate。`xpod_rdf_perm` 当前是
...
`perm_index_probe` 会直接读取 native index relation，按 leading prefix 统计 block seek、
page skip、page-local seek、entry match 和 posting match，用于验证 custom AM 的真实页访问行为。
...
或 join；排序/分页只是在 heap recheck SQL 外层保守执行，不是 native index-level
skip/order。单 pattern、非 DISTINCT 的 `COUNT` aggregate 在 native provider 声明
`index.xpod_rdf_perm.count` / `index.xpod_rdf_perm.count_any` 后会下沉到
`perm_index_count(_any)`：extension 直接扫描 custom index pages，按完整
graph/subject/predicate/object id filters 过滤，并通过 heap snapshot recheck 后返回 scalar count；
能力缺失或遇到 graph-prefix/range/text/excluded filter 时仍回退到
`perm_index_scan(_any) + COUNT(*) + heap recheck` SQL 路径，避免再进入 `execute_plan_json`。
这仍只是 scalar count fast path，不是完整 grouped/numeric custom aggregate executor。subject-star
count 只覆盖无 group / having / order / limit、无 `distinctVariables`、无 probe object range 的
count 子集；能力缺失或 shape 不匹配时继续回退到 `subject_star_join(...)` row stream + PG SQL
aggregate。`bgp_join` / `bgp_count` / `bgp_group_count` 只覆盖 1..4 个 pattern、最多
8 个变量、exact-id 常量/变量等值的 required BGP 行流；ORDER/LIMIT/OFFSET 由
`PostgresRdfEngine` 在外层 SQL 上执行，不是
native index-level early stop / ordered stream；supported grouped / numeric aggregate 可以复用
`bgp_join` 作为输入 row stream；非分组 count / count-distinct 可进一步走 `bgp_count`，
count-only grouped aggregate 可进一步走 `bgp_group_count`，但 numeric aggregate operator
本身还不是 native C executor；非 subject-star graph-prefix、无法映射的 term filter、range、
numeric native aggregate 仍回退。`xpod_rdf_perm` 仍尚未实现 block-level skip table、完整 custom index-aware join
算法和 custom aggregate operator，因此不作为完整性能收益结论。
...
extension 提供。安装 native extension 时，`scan.*` 可切到 `scan_quads`；ordinary required
BGP join 可切到 `bgp_join`；supported grouped / numeric aggregate 可以复用 native BGP
row stream，但 aggregate 计算保持 outer PG SQL，避免
`execute_plan_json` wrapper 带来的性能退化。后续 native extension 的职责是在不改
`PostgresRdfEngine` 物理计划语义的前提下，提供真实 custom join / aggregate 算法，而不是新增
第二套查询语义。
...
indexes。native extension 同时提供内部观测函数 `xpod_rdf.perm_index_stats(regclass)` 和
...
- native extension 只能声明已经真实接线的能力。当前 native provider 可声明
  ...
  term-id / graph-prefix scan；也可声明 `index.xpod_rdf_perm.scan`，对应 exact leading-prefix
  单 pattern 的 direct custom-index scan + heap recheck；可声明
  `index.xpod_rdf_perm.scan_any`，对应连续 leading prefix `$in` / scalar 混合数组的 direct
  custom-index scan + heap recheck；可声明 `index.xpod_rdf_perm.count` /
  `index.xpod_rdf_perm.count_any`，对应单 pattern 非 DISTINCT scalar count 的 native
  custom-index scan + heap visibility recheck；也可声明 `join.subject_star`，对应 native
  `subject_star_join(...)` 的受限 subject-star seed/probe/recheck path；可声明
  `aggregate.subject_star_count`，对应同一受限 subject-star shape 的 native count /
  count-distinct summary ABI；可声明 `join.required_bgp.native`，对应 `bgp_join(...)`
  的受限 exact-id required-BGP row-stream ABI；可声明 `join.values.native`，对应
  `values_join(...)` 的单 tuple VALUES source + exact-id required-BGP row-stream ABI；
  可声明 `aggregate.bgp_count`，对应 `bgp_count(...)` 的非分组 count / count-distinct
  summary ABI；可声明 `aggregate.bgp_group_count`，对应 `bgp_group_count(...)` 的 grouped
  count / count-distinct summary ABI。
  ORDER/LIMIT/OFFSET 由 outer SQL 消化。
  probe object range filter/order 由
  `PostgresRdfEngine` 的 outer SQL 完成，不进入 `subject_star_count`。`join.required_bgp`、
  `join.values`、`aggregate.count` 和 `aggregate.numeric` 仍可作为 engine-sql hot operator
  能力记录；`join.values` 当前表示 `VALUES` term-id inline relation 已下沉到
  `PostgresRdfEngine` 的 PG RDF-3X SQL join；`join.values.native` 才表示已走 native
  `xpod_rdf.values_join(...)`；`aggregate.bgp_count` 才表示非 subject-star BGP count 已走
  native summary；`aggregate.bgp_group_count` 才表示非 subject-star count-only grouped
  aggregate 已走 native summary。不能仅因 native extension 安装就代表 numeric aggregate
  计算已进入 native C executor。
...
- `capabilityProviders` 按能力位记录实际来源。当前混合 profile 中，`cache.result` 可以来自
  ...
  可以来自 native `extension`；`index.xpod_rdf_perm.scan`、
  `index.xpod_rdf_perm.scan_any`、`join.subject_star`、`aggregate.subject_star_count`
  `join.required_bgp.native` 和 `join.values.native` 只能来自 native `extension`；
  多源/不支持的 VALUES join fallback 和 aggregate 计算仍来自 outer `engine-sql`，直到 native custom executor
  通过 benchmark gate。
...
- 当前 native `0.1.0` 已实现最小 `scan.exact_graph` / `scan.graph_prefix` / `scan.term_in` ABI：
  ...
  `pg-custom-index` profile 还会在 native provider 声明
  `index.xpod_rdf_perm.scan_any` 后，把连续 leading prefix 的 `$in` term-id single-pattern 查询
  进一步路由到 `xpod_rdf.perm_index_scan_any(...) + heap recheck`。单 pattern 非 DISTINCT
  `COUNT` aggregate 在 native count capability 可用时会走
  `xpod_rdf.perm_index_count(_any)(...)`；否则复用 `perm_index_scan(_any) + heap
  ...
  plan 中标记 `XpodRdfSubjectStarJoin(subject_star_join)`；probe object range filter 和 order
  在 outer SQL 中通过 `rdf_terms` 执行。同一受限 shape 的非分组 count / count-distinct
  aggregate 在 `aggregate.subject_star_count` capability 可用时会走
  `xpod_rdf.subject_star_count(...)`，直接返回最多 8 个 count summary 列；遇到 group /
  having / order / limit、`distinctVariables` 或 probe object range 时回退到
  `subject_star_join(...)` row stream + PG SQL aggregate。`join.required_bgp.native` capability
  可用时，非 subject-star、无 filter 的 exact-id required BGP 子集会走
  `xpod_rdf.bgp_join(...)`，按 planner 给出的 join order 递归扫描 custom index 并返回最多
  8 个变量列；单个 tuple VALUES source 可走 `xpod_rdf.values_join(...)`，把 tuple 值预绑定到
  BGP 变量 slot 后再递归扫描 custom index；非分组 count / count-distinct 可走
  `xpod_rdf.bgp_count(...)`，直接返回最多 8 个 count summary 列；count-only grouped
  aggregate 可走 `xpod_rdf.bgp_group_count(...)`，直接返回最多 8 个 group key 列和 8 个
  count summary 列，HAVING / ORDER / LIMIT 在 outer SQL 继续执行。ORDER/LIMIT/OFFSET 和
  supported numeric aggregate 可在 native row stream 外层由 PG SQL 继续执行。
  `execute_plan_json` 保留为 legacy ABI，但 group aggregate 和 numeric
  aggregate 不再通过它执行；native C numeric aggregate executor 未完成前，numeric aggregate
  仍使用 PG SQL hot path 完成计算。
...
`prefixStatsExact` 未被乱序 append 清掉时使用这些 stats。`perm_index_probe` 已可不经 planner
SQL 直接验证 prefix probe 的 block seek / page skip / postings match。下一步还需要把这些
callback 补上 block-level skip hints 和 native hot-operator 接入。目标至少需要覆盖：
