...
`xpod_rdf.bgp_count(...)` narrow native BGP count / count-distinct summary ABI、`xpod_rdf_perm` custom index access method
...
直接在 native row stream 上返回 summary。
ORDER/LIMIT/OFFSET 可在 `bgp_join` / `values_join` row stream 外层 SQL 保守执行；
supported grouped / numeric aggregate 也可以把 aggregate 输入 BGP 流切到 native row stream，
但 group / numeric aggregate 计算仍由外层 PG SQL 完成；多源或暂不支持的 VALUES 形态仍回退
...
在 outer SQL 中 join `rdf_terms` 执行 recheck/filter/order。`xpod_rdf_perm` 当前是
...
aggregate。`bgp_join` 只覆盖最多 4 个 pattern、最多 8 个变量、exact-id 常量/变量等值的
required BGP 行流；ORDER/LIMIT/OFFSET 由 `PostgresRdfEngine` 在外层 SQL 上执行，不是
...
但 grouped / numeric aggregate operator 本身还不是 native C executor；
非 subject-star graph-prefix、无法映射的 term filter、range、grouped/numeric native aggregate 仍回退。`xpod_rdf_perm` 仍尚未实现 block-level skip table、完整 custom index-aware join
...
- native extension 只能声明已经真实接线的能力。当前 native provider 可声明
  ...
  native summary。不能仅因 native extension 安装就代表 grouped / numeric aggregate 计算已进入 native C executor。
...
- 当前 native `0.1.0` 已实现最小 `scan.exact_graph` / `scan.graph_prefix` / `scan.term_in` ABI：
  ...
  `xpod_rdf.bgp_count(...)`，直接返回最多 8 个 count summary 列；ORDER/LIMIT/OFFSET 和
  supported grouped / numeric aggregate 可在 native row stream 外层由 PG SQL 继续执行。
  `execute_plan_json` 保留为 legacy ABI，但 group aggregate 和 numeric
  aggregate 不再通过它执行；native C aggregate executor 未完成前，它们仍使用 PG SQL hot path
  完成 aggregate 计算。
