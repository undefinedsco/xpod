...
`PostgresRdfEngine` 解析好的 term-id exact / IN pattern，并且只在无排序、无分页、无 DISTINCT、
无同 pattern 变量相等约束的单 pattern 查询上接线；graph prefix、range/text filter、BGP join
和 aggregate 仍回退到 PG RDF-3X engine-sql path。`xpod_rdf_perm` 当前是
...
- native extension 只能声明已经真实接线的能力。当前 native provider 可声明
  `scan.exact_graph` / `scan.term_in`，对应 `scan_quads` 的单 pattern term-id scan；不能声明
  `scan.graph_prefix`、`join.required_bgp` 或 `aggregate.*`，这些能力仍由 `engine-sql`
  provider 提供。
...
- `capabilityProviders` 按能力位记录实际来源。当前混合 profile 中，`cache.result` 可以来自
  `sql-abi` 或 native `extension`；`scan.exact_graph` / `scan.term_in` 可以来自 native
  `extension`；graph-prefix scan / join / aggregate 在 native extension 替换前来自
  `engine-sql`。
...
- 当前 native `0.1.0` 已实现最小 `scan.exact_graph` / `scan.term_in` ABI：
  ...
  该 ABI 仍不是完整 native planner：排序、分页、DISTINCT、graph prefix、range/text filter、
  BGP join 和 aggregate 继续使用 PG RDF-3X baseline / engine-sql hot path。
