...
- native extension 只能声明已经真实接线的能力。当前 native provider 可声明
  ...
  `aggregate.count` 和 `aggregate.numeric` 仍可作为 engine-sql hot operator 能力记录，但不能
  仅因 native extension 安装就代表 ordinary join / aggregate 已进入 native executor。
...
- `capabilityProviders` 按能力位记录实际来源。当前混合 profile 中，`cache.result` 可以来自
  ...
  ordinary join / aggregate 仍来自 `engine-sql`，直到 native custom executor 通过 benchmark gate。
