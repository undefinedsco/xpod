...
解锁动作：Zero Trust → Networks → Tunnels → `linx-local` 复制**新** token 填入 `CLOUDFLARE_TUNNEL_TOKEN`；并把该隧道 public hostname（`node-0000.undefineds.co`）的 **Service（回源）** 指向运行时**隧道入口**——网络设置页显示的那一行可复制地址，默认 `127.0.0.1:<gateway+3>`（候选端口 3300 → 3303）。验收脚本按这条口径自动推算并校验；控制台填了别的端口时，结论里会写明该填哪个数。DNS 记录建议由 DNS only 改为 **Proxied**（Cloudflare 对 Tunnel 记录的告警即此）。
...
| N16 客户端打包分发 | **声明/解析/探测已落地（分发待决策）** | provider 目录已声明每个客户端（名字、指定路径的环境变量、安装提示、许可、**能否随产物分发**），`TunnelClientResolver` 统一解析顺序（显式路径 → 包内 `vendor/tunnel-clients/` → PATH），缺失报 `binary-missing:<provider>:<binary>` 并附安装提示，`scripts/check-tunnel-clients.ts` 可在验收前预检。**产物仍未携带客户端**：cloudflared 与上游 frpc 为 Apache-2.0 可分发，ngrok 专有、natfrp fork 无公开源码不可分发；打包的体积/签名策略属产品决策。见 [`network-audit-w2-w4-delivery.md`](network-audit-w2-w4-delivery.md) |
...
5. **W2–W4 已完成**（N03/N06/N07/N17、N14/N15/N18/N19，N12 与 N16 各差一个待决策的半边），逐项证据见 [`network-audit-w2-w4-delivery.md`](network-audit-w2-w4-delivery.md)，台账滚动状态见 [`network-audit-2026-09-19-reconciliation.md`](network-audit-2026-09-19-reconciliation.md) 第 0 节。仍未做：N16 的产物内置客户端（需定体积/签名策略）、N12 的 admin 侧真实探测（需定是否对外发探针）、N15 的 cluster 模式续期调度、N06 的会话内路由切换（failover）。
