...
`.co` 生产 QLever 集成已经按 `0.4.0-rc.3 -> 0.4.0` 完成发布。RC、生产替换和
stable finalization 是三个独立门禁；stable 没有重新构建镜像，也没有再次部署生产。
...
| 层次 | 当前事实 |
| --- | --- |
| Public Xpod source | `aaf911378f89faece05d7f307464e29c58421c52` |
| Private xpod-pro source | `ff604772d56186bc194643d9af85c01086930a16`，tag `v0.4.0` |
...
| Stable PG/QLever tag | `ccr.ccs.tencentyun.com/undefineds/xpod-rdf-postgres:0.4.0`，门禁验证它解析到上述 accepted digest |
...
生产只接受上述 RC evidence，不接受任意 tag 或重新输入镜像。替换过程先验证镜像和
...
Stable gate 只消费 immutable production evidence。它重新校验 candidate line、源码、
运行镜像、QLever conformance、active PVC 和 predecessor PVC，再用 registry manifest
retag 将 accepted PG digest 标记为 `0.4.0`。如果该 stable tag 已存在但指向不同 digest，
流程在改写前失败。整个 stable 阶段不访问 Kubernetes、不编译、不部署。
