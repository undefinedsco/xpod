...
The WebView page runs `@inrupt/solid-client-authn-browser`, logs into the Cloud OIDC issuer, and uses `session.fetch` to access an SP resource.
...
For Cloud/SP validation, set `issuer` to the Cloud issuer and `sp` to the SP resource URL.
