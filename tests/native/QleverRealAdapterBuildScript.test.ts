...
const scriptPath = path.join(repoRoot, 'scripts/check-qlever-real-adapter-build.cjs');
...
describe('QLever real adapter build script', () => {
  it('is exposed as a package script', () => {
    ...
    expect(pkg.scripts?.['check:qlever-real-adapter']).toBe('node scripts/check-qlever-real-adapter-build.cjs');
  });
