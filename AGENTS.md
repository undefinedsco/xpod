...
| 管理/运维 API | API Server (`src/api/handlers/`) | 普通 TypeScript 路由 | `SubdomainHandler`, `NodeHandler`, `QuotaHandler` |
...
### 带宽配额与限速
- Cloud 配置默认启用带宽统计：`UsageTrackingStore` 负责资源读写、`SubgraphSparqlHttpHandler` 负责 `.sparql` 入口，均会更新 `identity_usage` 表中的 `scope_type`、`scope_id`、`ingress_bytes`、`egress_bytes`。
- 默认限速 10 MiB/s（`config/cloud.json` 中的 `options_defaultAccountBandwidthLimitBps`），设置为 0 或删除该字段即表示不限速。
- `identity_usage.storage_limit_bytes` / `bandwidth_limit_bps` 通过 `scope_type = 'account' | 'pod'` 区分账号和 Pod 粒度；未来 Admin/桌面端可直接更新这些列完成覆写。
