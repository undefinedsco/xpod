...
export function resolveAiConnectionApiBase(podBaseUrl) {
...
export function createAiConnectionClient({ webId, podBaseUrl, authenticatedFetch, }) {
    const apiBase = resolveAiConnectionApiBase(podBaseUrl);
    const request = async (path, method, body, context = {}) => {
    ...
    const requestConnect = async (provider, path, method, body) => {
    ...
    return {
        ...
        apiBase,
        getServiceAccess() {
            return request('/api/applets/service-access/ai-connection', 'GET');
        },
        async listProviders() {
            const payload = await request('/api/ai/connections/providers', 'GET');
        ...
        completeApiKey(provider, attempt, apiKey, accountLabel) {
            return requestConnect(provider, '/connect/complete-api-key', 'POST', compactObject({
        ...
        async disconnect(provider) {
            const payload = await request(`${providerPath(provider)}/connect`, 'DELETE');
            return parseCredential(payload.record);
        },
        quota(provider, refresh = false) {
            return request(`${providerPath(provider)}/quota/${refresh ? 'refresh' : 'status'}`, refresh ? 'POST' : 'GET', refresh ? {} : undefined, { provider });
        },
    };
}
export function normalizeAiConnectionThrownError(error) {
    if (error instanceof Error) {
        return normalizeAiConnectionErrorText(error.message);
    }
    return normalizeAiConnectionErrorText(String(error));
}
export function normalizeAiConnectionErrorMessage(payload, status, context = {}) {
...
function normalizeAiConnectionErrorText(message) {
    ...
    if (prefixed)
        return prefixed;
    if (message === 'AI Connection service identity is unavailable')
        return message;
    if (message.startsWith('invalid_'))
        return 'AI Connection returned an invalid response.';
    return AI_CONNECTION_GENERIC_ERROR_MESSAGE;
}
...
function messageForSafeErrorCode(code, provider) {
...
function assertProvider(provider) {
    if (!AI_CONNECTION_PROVIDERS.includes(provider)) {
...
function providerValue(value) {
    ...
    if (normalized === 'alibaba' || normalized === 'dashscope')
        return 'bailian';
    return AI_CONNECTION_PROVIDERS.includes(normalized)
        ? normalized
        : undefined;
}
...
function parseConnectAttempt(value, expectedProvider) {
    if (!isRecord(value)
