...
export function PodsPage(): JSX.Element {
  ...
  useEffect(() => {
    ...
    async function load(): Promise<void> {
      ...
      setError(null);
      try {
        const response = await fetch('/admin/pods', { credentials: 'include' });
        if (!response.ok) {
          if (response.status === 401 || response.status === 403) {
            setError('Unauthorized. Please sign in with an administrator account.');
          } else {
            setError(`Failed to load pods (status ${response.status}).`);
          }
          return;
        }
        const payload = await response.json();
        if (!cancelled) {
          setPods(Array.isArray(payload.pods) ? payload.pods : []);
        }
      } catch (cause) {
        if (!cancelled) {
  ...
  return (
    <div className="space-y-4">
      <PageHeader title={t('pods.title')} subtitle={t('pods.summary')}>
        {error && (
                ...
                {pods.map((pod) => (
                  <tr key={pod.podId}>
                    <td className="px-4 py-3">
                      <div className="font-medium text-slate-900 dark:text-slate-100">{pod.podId}</div>
                    </td>
                    <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{pod.accountId ?? '—'}</td>
                    <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{pod.baseUrl ?? '—'}</td>
                    <td className="px-4 py-3 text-right text-slate-600 dark:text-slate-300">{formatBytes(pod.usedBytes)}</td>
                    <td className="px-4 py-3 text-right text-slate-600 dark:text-slate-300">{formatQuota(pod.quotaLimit)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
