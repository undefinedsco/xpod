  ...
  return <aside data-list-navigation className="h-full overflow-y-auto border-r border-border bg-muted/20 py-2">
    {groups.map((group) => <section key={group} className="mb-3">
      ...
      {statusNavigationItems.filter((item) => item.group === group).map((item) => {
        ...
        return <NavLink key={item.id} to={item.path} end={item.end} onKeyDown={handleListNavigationKeyDown} onClick={() => workspace.openMain()} className={({ isActive }) => clsx('mx-2 flex items-center gap-3 rounded-lg px-3 py-2 text-sm', isActive ? 'bg-accent font-medium text-accent-foreground' : 'text-foreground hover:bg-accent/60')}>
