...
describe('AI Config API', () => {
  test('loads the authenticated Pod policy', async () => {
    ...
    expect(fetchImpl).toHaveBeenCalledWith('/api/ai/config', expect.objectContaining({ method: 'GET', credentials: 'include' }));
  });
  ...
  test('patches only the edited policy section', async () => {
    ...
    expect(fetchImpl).toHaveBeenCalledWith('/api/ai/config', expect.objectContaining({
      method: 'PATCH',
  ...
  test('runs bounded chat and embedding model probes', async () => {
    ...
    expect(fetch).toHaveBeenNthCalledWith(1, '/v1/chat/completions', expect.objectContaining({
    ...
    expect(fetch).toHaveBeenNthCalledWith(2, '/v1/embeddings', expect.objectContaining({
