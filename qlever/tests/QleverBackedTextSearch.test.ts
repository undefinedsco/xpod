...
const textSearchHeader = path.join(repoRoot, 'qlever/qlever_adapter/src/XpodBackedTextSearch.hpp');
...
`, 'utf8');
  ...
  it('fails closed when text search capability is not declared', async () => {
    expect(hasCxx(), 'c++ compiler is required for native backed text search capability check').toBe(true);

    const root = await mkdtemp(path.join(os.tmpdir(), 'xpod-backed-text-search-missing-capability-'));
    try {
      const smoke = path.join(root, 'backed_text_search_missing_capability_smoke.cpp');
      const binary = path.join(root, 'backed_text_search_missing_capability_smoke');
      await writeFile(smoke, `
#include "XpodBackedTextSearch.hpp"
