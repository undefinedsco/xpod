...
const qleverInclude = path.join(repoRoot, 'qlever/include');
...
class Id {
 public:
  ...
  static Id makeFromLocalVocabIndex(uint64_t index) { return Id(index + 1000000); }
...
describe('QLever candidate physical operation bridge', () => {
  nativeIt('executes text and vector candidate roots from a native physical plan', async () => {
...
static xpod_rdf_status text_search(
  ...
  row.retrieval_point_key = {"chunk-alpha", 11};
  row.has_source_key = 1;
  row.source_key = {"source-alpha", 12};
  row.score = 0.8;
  xpod_rdf_text_term_key matched_term = 303;
  uint8_t has_matched_term = 1;
  xpod_rdf_candidate_batch batch = {};
  batch.rows = &row;
  batch.row_count = 1;
  batch.scanned_rows = 9;
  batch.scorer = {"pg-ts-rank-cd", 13};
  batch.matched_terms = &matched_term;
  batch.has_matched_terms = &has_matched_term;
...
int main() {
  ...
  if (!text_result.candidates.rows[0].has_matched_term ||
      text_result.candidates.rows[0].matched_term != 303) return 35;
  ...
  vector.request.provider = {"xpod", 4};
  vector.request.model = {"embed-v1", 8};
  vector.request.model_version = {"2026-08-12", 10};
  vector.request.input_kind = {"entity-card", 11};
  vector.request.projection_policy_version = {"policy-v1", 9};
...
`, 'utf8');
  ...
  nativeIt('fails closed when declared candidate output columns are missing', async () => {
    expect(hasCxx(), 'c++ compiler is required for native candidate operation bridge check').toBe(true);
...
static xpod_rdf_status text_search(
  ...
  row.retrieval_point = 101;
  row.has_retrieval_point_key = 1;
  row.retrieval_point_key = {"chunk-entity", 12};
  row.score = 0.8;
  xpod_rdf_candidate_batch batch = {};
  batch.rows = &row;
  batch.row_count = 1;
...
`, 'utf8');
  ...
  nativeIt('fails closed when declared vector candidate output columns are missing', async () => {
    expect(hasCxx(), 'c++ compiler is required for native candidate operation bridge check').toBe(true);

    const root = await mkdtemp(path.join(os.tmpdir(), 'xpod-qlever-vector-candidate-columns-'));
    try {
      const qleverSource = await writeFakeQleverHeaders(root);
      const smoke = path.join(root, 'vector_candidate_columns_smoke.cpp');
      const binary = path.join(root, 'vector_candidate_columns_smoke');
      await writeFile(smoke, `
#include "XpodQleverOperationExecutor.hpp"
...
static xpod_rdf_status vector_search(
  ...
  xpod_rdf_candidate row = {};
  row.has_retrieval_point = 1;
  row.retrieval_point = 303;
  row.has_retrieval_point_key = 1;
  row.retrieval_point_key = {"chunk-vector", 12};
  row.score = 0.91;
  xpod_rdf_candidate_batch batch = {};
  batch.rows = &row;
  batch.row_count = 1;
...
int main() {
  ...
  xpod::qlever::BridgePhysicalPlan plan;
  xpod::qlever::BridgeVectorCandidateSource source;
  source.output_columns.push_back({
      "point",
      xpod::qlever::BridgeCandidateColumnKind::RetrievalPoint,
  });
  ...
  plan.vector_sources.push_back(source);
  plan.root.kind = xpod::qlever::BridgeOperationKind::VectorSearch;
  plan.root.candidate_index = 0;
