...
const bridgeHeader = path.join(repoRoot, 'qlever/qlever_adapter/src/XpodQleverPhysicalTextIndexScanContextBridge.hpp');
...
class Id {
 public:
  ...
  static Id makeFromTextRecordIndex(TextRecordIndex index) {
    return Id((uint64_t{6} << 60) | index.get());
  }
  static Id makeFromWordVocabIndex(WordVocabIndex index) {
    return Id((uint64_t{7} << 60) | index.get());
  }
...
describe('QLever physical text index scan context bridge', () => {
  it('tells physical backends whether QLever needs records or entity mentions', async () => {
    const source = await readFile(bridgeHeader, 'utf8');

    expect(source).toContain('request.candidate_kind = XPOD_RDF_TEXT_CANDIDATE_RECORD');
    expect(source).toContain('request.candidate_kind = XPOD_RDF_TEXT_CANDIDATE_ENTITY');
  });

  it('materializes exact and prefix TextIndexScanForWord from the injected physical text source', async () => {
...
static uint64_t text_record_bits(uint64_t key) {
  return Id::makeFromTextRecordIndex(TextRecordIndex::make(key)).getBits();
}

static uint64_t word_vocab_bits(uint64_t key) {
  return Id::makeFromWordVocabIndex(WordVocabIndex::make(key)).getBits();
}
...
static xpod_rdf_status estimate_text_search(
  ...
  const std::string_view query{request->query.data, request->query.size};
  if (query != "runtime" && query != "common" && query != "rare" &&
      query != "run*") {
    return XPOD_RDF_STATUS_BACKEND_ERROR;
  }
  ...
  out_estimate->rows = query == "common" ? 100
                       : query == "rare" ? 1
                       : request->required_entities_size == 1 ? 1 : 2;
...
static xpod_rdf_status text_search(
  ...
  rows[1].score = 0.25;
  xpod_rdf_text_term_key matched_terms[2] = {51, 52};
  uint8_t has_matched_terms[2] = {
      static_cast<uint8_t>(query == "run*"),
      static_cast<uint8_t>(query == "run*")};
  xpod_rdf_candidate_batch batch = {};
  batch.rows = rows;
  batch.row_count = request->required_entities_size == 1 ? 1 : 2;
  batch.scanned_rows = 9;
  batch.matched_terms = matched_terms;
  batch.has_matched_terms = has_matched_terms;
...
int main() {
  ...
  const std::vector<std::string> potential_terms{"common", "rare", "runtime"};
  auto best_term = xpod::qlever::bestPhysicalTextTermIndexFromContext(
      qec, potential_terms);
  if (!best_term.has_value() || *best_term != 1) return 42;
  ...
  if (result.result.idTable()(0, 0).getBits() != text_record_bits(41)) return 4;
  if (result.result.idTable()(1, 0).getBits() != text_record_bits(42)) return 5;
  ...
  auto prefix_estimate = xpod::qlever::textWordSizeEstimateFromContext(
      qec, "run*", true, false);
  if (prefix.status != XPOD_RDF_STATUS_OK ||
      prefix_estimate.status != XPOD_RDF_STATUS_OK) return 8;
  if (prefix_estimate.rows != 2 || prefix_estimate.cost != 12 ||
      !prefix_estimate.exact) return 43;
  if (prefix.result.idTable().numColumns() != 2 ||
      prefix.result.idTable().numRows() != 2) return 44;
  if (prefix.result.idTable()(0, 0).getBits() != text_record_bits(41) ||
      prefix.result.idTable()(0, 1).getBits() != word_vocab_bits(51) ||
      prefix.result.idTable()(1, 0).getBits() != text_record_bits(42) ||
      prefix.result.idTable()(1, 1).getBits() != word_vocab_bits(52)) return 45;
  if (text_search_calls != 2) return 46;
  ...
  if (score.result.idTable()(0, 0).getBits() != text_record_bits(41)) return 11;
  if (score.result.idTable()(0, 1).getBits() != 9750) return 33;
  if (score.result.idTable()(1, 0).getBits() != text_record_bits(42)) return 34;
  if (score.result.idTable()(1, 1).getBits() != 9250) return 35;
  if (text_search_calls != 3) return 36;

  xpod_rdf_backend_v1 legacy_text_backend = raw_backend;
  legacy_text_backend.get_capabilities = [](
      void*, xpod_rdf_backend_capabilities* out) {
    out->features = XPOD_RDF_BACKEND_FEATURE_TEXT_SEARCH;
    return XPOD_RDF_STATUS_OK;
  };
  xpod::rdf::PhysicalBackend legacy_text_physical(&legacy_text_backend);
  QueryExecutionContext legacy_text_qec;
  legacy_text_qec.setXpodPhysicalIndex(
      make_index(legacy_text_physical, request));
  const int calls_before_legacy_prefix = text_search_calls;
  auto legacy_prefix = xpod::qlever::textWordResultFromContext(
      legacy_text_qec, "run*", true, false, {ColumnIndex{0}});
  auto legacy_prefix_estimate = xpod::qlever::textWordSizeEstimateFromContext(
      legacy_text_qec, "run*", true, false);
  if (legacy_prefix.status != XPOD_RDF_STATUS_UNSUPPORTED ||
      legacy_prefix_estimate.status != XPOD_RDF_STATUS_UNSUPPORTED) return 47;
  if (text_search_calls != calls_before_legacy_prefix) return 48;
  ...
  if (variable_entity.result.idTable()(0, 0).getBits() != text_record_bits(41)) return 20;
  if (variable_entity.result.idTable()(0, 1).getBits() != 1051) return 21;
  if (variable_entity.result.idTable()(1, 0).getBits() != text_record_bits(42)) return 22;
  ...
  if (fixed_entity.result.idTable()(0, 0).getBits() != text_record_bits(41)) return 29;
  ...
  if (entity_score.result.idTable()(0, 0).getBits() != text_record_bits(41)) return 39;
