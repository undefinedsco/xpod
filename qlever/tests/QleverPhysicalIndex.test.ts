...
const physicalIndexHeader = path.join(repoRoot, 'qlever/qlever_adapter/src/XpodQleverPhysicalIndex.hpp');
...
describe('Xpod-backed QLever physical index seam', () => {
  it('binds a fixed QLever component even when its optional vocabulary id is absent', async () => {
    ...
    const root = await mkdtemp(path.join(os.tmpdir(), 'xpod-qlever-fixed-component-'));
    try {
      const qleverSource = await writeMinimalQleverHeaders(root);
      const smoke = path.join(root, 'fixed_component_smoke.cpp');
      const binary = path.join(root, 'fixed_component_smoke');
...
int main() {
  ...
  std::optional<Id> missing_id;
  Component fixed_subject;
  auto status = xpod::qlever::applyQleverScanSpecColumn(
      pattern, context, 'S', missing_id, &fixed_subject);
...
`, 'utf8');
  ...
  it('uses backend scan cursors lazily for physical lazy scan ranges', async () => {
    expect(hasCxx(), 'c++ compiler is required for native physical lazy cursor check').toBe(true);

    const root = await mkdtemp(path.join(os.tmpdir(), 'xpod-qlever-physical-lazy-cursor-'));
    try {
      const qleverSource = await writeMinimalQleverHeaders(root);
      const smoke = path.join(root, 'physical_lazy_cursor_smoke.cpp');
      const binary = path.join(root, 'physical_lazy_cursor_smoke');
      await writeFile(smoke, `
#include <stdexcept>
#include <vector>
#include "XpodQleverPhysicalIndex.hpp"
...
static xpod_rdf_status open_scan_cursor(
  ...
  if (request->pattern.has_subject != 1 || request->pattern.subject != 11) {
    return XPOD_RDF_STATUS_BACKEND_ERROR;
  }
...
int main() {
  ...
  raw_backend.open_scan_cursor = open_scan_cursor;
  raw_backend.next_scan_cursor = next_scan_cursor;
  raw_backend.close_scan_cursor = close_scan_cursor;
  ...
  xpod::qlever::XpodQleverScanSpecAndBlocks spec = {};
  spec.pattern.has_subject = true;
  spec.pattern.subject = 11;
  spec.needed_slots = XPOD_RDF_SLOT_SUBJECT | XPOD_RDF_SLOT_OBJECT;

  {
    auto range = index.permutation(Permutation::Enum::SPO).lazyScanRange(spec);
    if (range.status != XPOD_RDF_STATUS_OK) return 1;
    if (state.open_calls != 0 || state.next_calls != 0 || state.scan_calls != 0) return 2;
    ...
    if (state.open_calls != 1 || state.next_calls != 1 || state.scan_calls != 0) return 6;
  }
...

  {
    ...
    if (state.next_calls != 5) return 12;
  }
  if (state.close_calls != 2) return 13;

  state.next_error = XPOD_RDF_STATUS_BACKEND_ERROR;
  try {
    auto range = index.permutation(Permutation::Enum::SPO).lazyScanRange(spec);
  ...
  if (state.close_calls != 3) return 15;
  if (state.scan_calls != 0) return 16;
  return 0;
}
`, 'utf8');
...
static xpod_rdf_status text_search(
  ...
  xpod_rdf_candidate_batch batch = {
      &row, 1, 1, {"text", 4}, nullptr, nullptr};
  return on_batch(callback_user_data, &batch);
}
...
static xpod_rdf_status vector_search(
  ...
  row.score = 0.5;
  xpod_rdf_candidate_batch batch = {
      &row, 1, 1, {"vector", 6}, nullptr, nullptr};
  return on_batch(callback_user_data, &batch);
}
...
int main() {
  ...
  ScanSpecification spec{Id::fromBits(1022), Id::fromBits(1033), std::nullopt};
  ...
  if (state.decode_calls != 6) return 14;
  return 0;
}
`, 'utf8');
...
`, 'utf8');
  ...
  it('supports QLever GRAPH variable scans that blacklist the internal default graph', async () => {
    expect(hasCxx(), 'c++ compiler is required for native physical index graph-variable filter check').toBe(true);

    const root = await mkdtemp(path.join(os.tmpdir(), 'xpod-qlever-physical-index-graph-variable-'));
    try {
      const qleverSource = await writeMinimalQleverHeaders(root);
      const smoke = path.join(root, 'physical_index_graph_variable_smoke.cpp');
      const binary = path.join(root, 'physical_index_graph_variable_smoke');
      await writeFile(smoke, `
#include <optional>
#include <string_view>
#include <variant>
...
int main() {
  ...
  access_scope.denied_graphs = &denied_graph;
  access_scope.denied_graphs_size = 1;
  xpod_qlever_query_request request = {};
  request.access_scope = &access_scope;
  xpod::qlever::PlannerRequestContext context{physical, &request, request.cancellation};
  xpod::qlever::XpodQleverPhysicalIndex index(context);

  // QLever represents GRAPH ?g as a scan over named graphs, implemented as a
  // blacklist of its internal default graph special ID (bits=3).
  ScanSpecification spec{
      Id::fromBits(10),
      Id::fromBits(20),
      Id::fromBits(30),
      GraphFilter::Blacklist(Id::fromBits(3))};
  auto scan = index.scanScanSpecification(
      Permutation::Enum::SPO,
      spec,
      XPOD_RDF_SLOT_SUBJECT | XPOD_RDF_SLOT_GRAPH);
  if (scan.status != XPOD_RDF_STATUS_OK) return 1;
  if (scan.table.numColumns() != 2) return 2;
  if (scan.table.numRows() != 1) return 3;
...
`, 'utf8');
  ...
  it('rejects cursor lazy ranges before opening when scan capabilities are missing', async () => {
    expect(hasCxx(), 'c++ compiler is required for native physical cursor capability check').toBe(true);

    const root = await mkdtemp(path.join(os.tmpdir(), 'xpod-qlever-physical-cursor-capability-'));
    try {
      const qleverSource = await writeMinimalQleverHeaders(root);
      const smoke = path.join(root, 'physical_cursor_capability_smoke.cpp');
      const binary = path.join(root, 'physical_cursor_capability_smoke');
      await writeFile(smoke, `
#include "XpodQleverPhysicalIndex.hpp"
...
static xpod_rdf_status get_capabilities_without_spo(
    void*,
    xpod_rdf_backend_capabilities* out_capabilities) {
  out_capabilities->supported_permutations = XPOD_RDF_PERM_CAP_POSG;
  out_capabilities->features = XPOD_RDF_BACKEND_FEATURE_BLOCK_RESTRICTED_SCAN;
...
static void close_scan_cursor(void*, xpod_rdf_scan_cursor*) {}

static xpod_rdf_backend_v1 makeBackend(
    BackendState& state,
...
int main() {
  ...
  BackendState no_block_state = {};
  xpod_rdf_backend_v1 no_block_backend = makeBackend(
      no_block_state, get_capabilities_without_block_restricted_scan);
  ...
  xpod::qlever::PlannerRequestContext no_block_context{
      no_block_physical, &no_block_request, no_block_request.cancellation};
  ...
  auto selected_unsupported = no_block_index.permutation(Permutation::Enum::SPO)
      .lazyScanRange(scan_spec, {selected_block});
  if (selected_unsupported.status != XPOD_RDF_STATUS_UNSUPPORTED) return 1;
  if (selected_unsupported.blocks.has_value()) return 2;
  if (no_block_state.open_calls != 0 ||
      no_block_state.next_calls != 0 ||
      no_block_state.scan_calls != 0) return 3;

  BackendState no_perm_state = {};
  xpod_rdf_backend_v1 no_perm_backend = makeBackend(
      no_perm_state, get_capabilities_without_spo);
  xpod::rdf::PhysicalBackend no_perm_physical(&no_perm_backend);
  xpod_qlever_query_request no_perm_request = {};
  xpod::qlever::PlannerRequestContext no_perm_context{
      no_perm_physical, &no_perm_request, no_perm_request.cancellation};
  no_perm_context.capabilities_status =
      no_perm_physical.getCapabilities(no_perm_context.capabilities);
  xpod::qlever::XpodQleverPhysicalIndex no_perm_index(no_perm_context);
  auto permutation_unsupported = no_perm_index.permutation(Permutation::Enum::SPO)
      .lazyScanRange(scan_spec);
  if (permutation_unsupported.status != XPOD_RDF_STATUS_UNSUPPORTED) return 4;
  if (permutation_unsupported.blocks.has_value()) return 5;
  if (no_perm_state.open_calls != 0 ||
      no_perm_state.next_calls != 0 ||
      no_perm_state.scan_calls != 0) return 6;
  return 0;
}
`, 'utf8');
