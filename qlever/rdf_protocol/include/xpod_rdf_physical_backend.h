...
#define XPOD_RDF_PHYSICAL_BACKEND_VERSION_MINOR 20
...
typedef uint64_t xpod_rdf_text_term_key;
...
/* Kind-tagged keys fit entirely inside the 60-bit execution payload. */
typedef enum xpod_rdf_term_kind_tag {
  XPOD_RDF_TERM_KIND_TAG_RESERVED = 0,
  XPOD_RDF_TERM_KIND_TAG_IRI = 1,
  XPOD_RDF_TERM_KIND_TAG_BLANK = 2,
  XPOD_RDF_TERM_KIND_TAG_LITERAL = 3
} xpod_rdf_term_kind_tag;

#define XPOD_RDF_KIND_TAGGED_KIND_MASK UINT64_C(3)
#define XPOD_RDF_KIND_TAGGED_SEQUENCE_SHIFT 2
#define XPOD_RDF_KIND_TAGGED_MAX_SEQUENCE ((UINT64_C(1) << 58) - UINT64_C(1))

static inline uint8_t xpod_rdf_make_kind_tagged_term_key(
    uint64_t sequence,
    xpod_rdf_term_kind_tag kind,
    xpod_rdf_term_key* out) {
  if (out == NULL || sequence == 0 ||
      sequence > XPOD_RDF_KIND_TAGGED_MAX_SEQUENCE ||
      kind == XPOD_RDF_TERM_KIND_TAG_RESERVED ||
      kind > XPOD_RDF_TERM_KIND_TAG_LITERAL) {
    return 0;
  }
  *out = (sequence << XPOD_RDF_KIND_TAGGED_SEQUENCE_SHIFT) |
      (uint64_t) kind;
  return 1;
}

static inline xpod_rdf_term_kind_tag xpod_rdf_kind_tagged_term_key_kind(
    xpod_rdf_term_key key) {
  return (xpod_rdf_term_kind_tag) (key & XPOD_RDF_KIND_TAGGED_KIND_MASK);
}

static inline uint64_t xpod_rdf_kind_tagged_term_key_sequence(
    xpod_rdf_term_key key) {
  return key >> XPOD_RDF_KIND_TAGGED_SEQUENCE_SHIFT;
}
...
typedef struct xpod_rdf_quad {
  xpod_rdf_term subject;
  xpod_rdf_term predicate;
  xpod_rdf_term object;
  xpod_rdf_term graph;
  uint8_t has_graph;
} xpod_rdf_quad;

typedef enum xpod_rdf_mutation_kind {
  XPOD_RDF_MUTATION_INSERT = 1,
  XPOD_RDF_MUTATION_DELETE = 2
} xpod_rdf_mutation_kind;

typedef struct xpod_rdf_quad_mutation {
  xpod_rdf_mutation_kind kind;
  xpod_rdf_quad quad;
} xpod_rdf_quad_mutation;
...
typedef enum xpod_rdf_backend_feature {
  ...
  XPOD_RDF_BACKEND_FEATURE_BLOCK_RESTRICTED_SCAN = 1u << 11,
  XPOD_RDF_BACKEND_FEATURE_MUTATION = 1u << 12,
  XPOD_RDF_BACKEND_FEATURE_TRANSACTIONS = 1u << 13,
  XPOD_RDF_BACKEND_FEATURE_LOAD_DOCUMENT = 1u << 14,
  XPOD_RDF_BACKEND_FEATURE_TEXT_MATCHED_TERM = 1u << 15,
  XPOD_RDF_BACKEND_FEATURE_SEMANTIC_ORDER_SCAN = 1u << 16,
  XPOD_RDF_BACKEND_FEATURE_TERM_TUPLE_FILTER = 1u << 17,
  XPOD_RDF_BACKEND_FEATURE_SCAN_FILTER = 1u << 18,
  XPOD_RDF_BACKEND_FEATURE_SCAN_VALUE_RANGE = 1u << 19
...
#define XPOD_RDF_SCAN_ORDER_MAX_SLOTS 4

typedef struct xpod_rdf_scan_order {
  uint32_t slots[XPOD_RDF_SCAN_ORDER_MAX_SLOTS];
  xpod_rdf_scan_order_kind kinds[XPOD_RDF_SCAN_ORDER_MAX_SLOTS];
  size_t count;
} xpod_rdf_scan_order;
...
typedef struct xpod_rdf_term_tuple_filter {
  ...
  size_t row_count;
} xpod_rdf_term_tuple_filter;

typedef enum xpod_rdf_scan_filter_kind {
  XPOD_RDF_SCAN_FILTER_TERM_NOT_EQUAL = 1,
  XPOD_RDF_SCAN_FILTER_VALUE_GREATER_THAN = 2,
  XPOD_RDF_SCAN_FILTER_VALUE_GREATER_THAN_OR_EQUAL = 3,
  XPOD_RDF_SCAN_FILTER_VALUE_LESS_THAN = 4,
  XPOD_RDF_SCAN_FILTER_VALUE_LESS_THAN_OR_EQUAL = 5,
  XPOD_RDF_SCAN_FILTER_LANGUAGE_EQUAL = 6,
  XPOD_RDF_SCAN_FILTER_DATATYPE_EQUAL = 7,
  XPOD_RDF_SCAN_FILTER_STRING_PREFIX = 8,
  XPOD_RDF_SCAN_FILTER_STRING_CONTAINS = 9,
  XPOD_RDF_SCAN_FILTER_STRING_SUFFIX = 10,
  XPOD_RDF_SCAN_FILTER_STRING_EQUAL = 11
} xpod_rdf_scan_filter_kind;

typedef enum xpod_rdf_scan_filter_value_transform {
  XPOD_RDF_SCAN_FILTER_VALUE_IDENTITY = 0,
  XPOD_RDF_SCAN_FILTER_VALUE_LOWERCASE = 1,
  XPOD_RDF_SCAN_FILTER_VALUE_UPPERCASE = 2
} xpod_rdf_scan_filter_value_transform;

typedef struct xpod_rdf_scan_filter {
  uint32_t slot;
  xpod_rdf_scan_filter_kind kind;
  xpod_rdf_term_key term;
  xpod_rdf_bytes value;
  xpod_rdf_scan_filter_value_transform value_transform;
  uint8_t negated;
  xpod_rdf_term operand;
  uint8_t has_operand;
} xpod_rdf_scan_filter;
...
typedef struct xpod_rdf_scan_cursor xpod_rdf_scan_cursor;
...
typedef enum xpod_rdf_text_candidate_kind {
  XPOD_RDF_TEXT_CANDIDATE_RECORD = 1,
  XPOD_RDF_TEXT_CANDIDATE_ENTITY = 2
} xpod_rdf_text_candidate_kind;
...
typedef struct xpod_rdf_mutation_request {
  ...
  xpod_rdf_source_scope source_scope;
  const xpod_rdf_access_scope* access_scope;
  const xpod_rdf_quad_mutation* mutations;
  size_t mutation_count;
} xpod_rdf_mutation_request;

typedef struct xpod_rdf_mutation_result {
  uint64_t inserted_count;
  uint64_t deleted_count;
  xpod_rdf_bytes facts_version;
} xpod_rdf_mutation_result;

typedef struct xpod_rdf_load_document_request {
  xpod_rdf_snapshot snapshot;
  const xpod_rdf_cancellation* cancellation;
  xpod_rdf_bytes source_iri;
  xpod_rdf_term target_graph;
  uint8_t has_target_graph;
  xpod_rdf_source_scope source_scope;
  const xpod_rdf_access_scope* access_scope;
} xpod_rdf_load_document_request;

typedef struct xpod_rdf_load_document_result {
  xpod_rdf_bytes content;
  xpod_rdf_bytes media_type;
  xpod_rdf_bytes document_iri;
} xpod_rdf_load_document_result;
...
typedef struct xpod_rdf_candidate {
  ...
  xpod_rdf_bytes source_key;
  uint8_t has_source_key;
  xpod_rdf_bytes retrieval_point_key;
  uint8_t has_retrieval_point_key;
} xpod_rdf_candidate;
...
typedef struct xpod_rdf_candidate_batch {
  ...
  uint64_t scanned_rows;
  xpod_rdf_bytes scorer;
  const xpod_rdf_text_term_key* matched_terms;
  const uint8_t* has_matched_terms;
...
typedef xpod_rdf_status (*xpod_rdf_resolve_retrieval_points_fn)(
    void* backend_user_data,
...
typedef xpod_rdf_status (*xpod_rdf_resolve_text_term_fn)(
    void* backend_user_data,
    xpod_rdf_text_term_key key,
    const xpod_rdf_snapshot* snapshot,
    xpod_rdf_bytes* out_term);

typedef xpod_rdf_status (*xpod_rdf_resolve_text_terms_fn)(
    void* backend_user_data,
...
typedef xpod_rdf_status (*xpod_rdf_open_scan_cursor_fn)(
    void* backend_user_data,
    const xpod_rdf_scan_request* request,
    xpod_rdf_scan_cursor** out);

/*
 * Returns OK with a batch, DONE when the cursor is exhausted, CANCELLED when
 * cancellation interrupts the scan, UNSUPPORTED when this cursor cannot serve
 * the request, or BACKEND_ERROR for backend failures. Batch rows are owned by
 * the backend/cursor and remain valid only until the next next_scan_cursor call
 * on the same cursor or close_scan_cursor.
 */
typedef xpod_rdf_status (*xpod_rdf_next_scan_cursor_fn)(
    void* backend_user_data,
    xpod_rdf_scan_cursor* cursor,
    xpod_rdf_quad_batch* out);

/* close_scan_cursor must accept NULL and treat it as a no-op. */
typedef void (*xpod_rdf_close_scan_cursor_fn)(
    void* backend_user_data,
    xpod_rdf_scan_cursor* cursor);
...
typedef xpod_rdf_status (*xpod_rdf_apply_mutation_fn)(
    void* backend_user_data,
    const xpod_rdf_mutation_request* request,
    xpod_rdf_mutation_result* out_result);

typedef xpod_rdf_status (*xpod_rdf_load_document_fn)(
    void* backend_user_data,
    const xpod_rdf_load_document_request* request,
    xpod_rdf_load_document_result* out_result);

typedef xpod_rdf_status (*xpod_rdf_begin_transaction_fn)(
    void* backend_user_data,
    const xpod_rdf_snapshot* snapshot);

typedef xpod_rdf_status (*xpod_rdf_commit_transaction_fn)(
    void* backend_user_data);

typedef xpod_rdf_status (*xpod_rdf_rollback_transaction_fn)(
    void* backend_user_data);
...
typedef xpod_rdf_status (*xpod_rdf_prefetch_qlever_ids_fn)(
    void* backend_user_data,
    const xpod_rdf_term_key* terms,
    size_t term_count,
    const xpod_rdf_snapshot* snapshot);

typedef xpod_rdf_status (*xpod_rdf_encode_qlever_ids_fn)(
    void* backend_user_data,
    const xpod_rdf_term_key* terms,
    size_t term_count,
    const xpod_rdf_snapshot* snapshot,
...
typedef xpod_rdf_status (*xpod_rdf_compare_qlever_ids_fn)(
    void* backend_user_data,
...
typedef struct xpod_rdf_backend_v1 {
  ...
  xpod_rdf_resolve_retrieval_points_fn resolve_retrieval_points;
  ...
  xpod_rdf_resolve_text_term_fn resolve_text_term;
  xpod_rdf_resolve_text_terms_fn resolve_text_terms;
  xpod_rdf_prefetch_qlever_ids_fn prefetch_qlever_ids;
  xpod_rdf_encode_qlever_ids_fn encode_qlever_ids;
  xpod_rdf_open_scan_cursor_fn open_scan_cursor;
  xpod_rdf_next_scan_cursor_fn next_scan_cursor;
  xpod_rdf_close_scan_cursor_fn close_scan_cursor;
} xpod_rdf_backend_v1;
