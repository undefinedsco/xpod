...
export class ManagedRunWorker<TContext = StoreContext> {
  ...
  private async loadRunInput(runId: string, context: TContext): Promise<LoadedRunInput<TContext>> {
    ...
    return {
      ...
      threadRef,
      userMessage,
      runtimeConfig: this.resolveRuntimeConfig(run, thread, context),
      context,
    };
  }
