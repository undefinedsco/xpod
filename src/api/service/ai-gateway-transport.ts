...
import {
  getSharedPlatformModels,
  isSharedPlatformModel,
  type PlatformModelListItem,
} from './platform-ai-config';

interface AiGatewayModelCache {
  fetchedAt: number;
  items: PlatformModelListItem[];
}

type AiGatewayTimeoutKind = 'query' | 'generation';

function mergePlatformModels(
  first: PlatformModelListItem[],
  second: PlatformModelListItem[],
): PlatformModelListItem[] {
  const byId = new Map<string, PlatformModelListItem>();
...
export class AiGatewayTransport {
  ...
  public async shouldHandleModel(model?: string): Promise<boolean> {
    return isSharedPlatformModel(model);
  }

  public async listModels(): Promise<PlatformModelListItem[]> {
    const sharedModels = getSharedPlatformModels();
    const cache = await this.getModelCache();
    return mergePlatformModels(cache?.items ?? [], sharedModels);
  }
  ...
  private async getModelCache(): Promise<AiGatewayModelCache | null> {
    ...
    this.modelCachePromise = (async() => {
      ...
      const data = await response.json() as { data?: PlatformModelListItem[] };
      const items = Array.isArray(data.data)
        ? data.data.filter((item) => isSharedPlatformModel(item.id))
        : [];
